#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-20 verdict — cites the tranche-19 verdict + coverage delta,
...
V=docs/spec/ph-33-verdict.md
echo "== PH-18D exit-criteria (tranche-20 verdict; human gate) =="
...
must "verdict chains from tranche-19 verdict::ph-32-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS02 PS07 PS11; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-33D met (await HUMAN gate review)' || echo 'RED - PH-33D not complete') =="; exit "$fail"
