#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-21 verdict — cites the tranche-20 verdict + coverage delta,
...
V=docs/spec/ph-34-verdict.md
echo "== PH-18D exit-criteria (tranche-21 verdict; human gate) =="
...
must "verdict chains from tranche-20 verdict::ph-33-verdict" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-34D met (await HUMAN gate review)' || echo 'RED - PH-34D not complete') =="; exit "$fail"
