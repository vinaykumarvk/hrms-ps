#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-8 verdict — cites the tranche-7 verdict + coverage delta,
...
V=docs/spec/ph-21-verdict.md
echo "== PH-18D exit-criteria (tranche-8 verdict; human gate) =="
...
must "verdict chains from tranche-7 verdict::ph-20-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS07 PS08 PS09; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-21D met (await HUMAN gate review)' || echo 'RED - PH-21D not complete') =="; exit "$fail"
