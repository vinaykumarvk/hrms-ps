#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-9 verdict — cites the tranche-8 verdict + coverage delta,
...
V=docs/spec/ph-22-verdict.md
echo "== PH-18D exit-criteria (tranche-9 verdict; human gate) =="
...
must "verdict chains from tranche-8 verdict::ph-21-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS08 PS13 PS14; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-22D met (await HUMAN gate review)' || echo 'RED - PH-22D not complete') =="; exit "$fail"
