#!/usr/bin/env bash
# PH-38B oracle (human gate): honest tranche-25 verdict — cites the tranche-24 verdict + coverage delta,
# names the coverage ratchet (392->397 / 30%) and the calibration route exposure, remaining-gaps,
...
V=docs/spec/ph-38-verdict.md
echo "== PH-38B exit-criteria (tranche-25 verdict; human gate) =="
...
must "verdict chains from tranche-24 verdict::ph-37-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
must "verdict names the coverage ratchet::397|30%|calibration" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-38B met (await HUMAN gate review)' || echo 'RED - PH-38B not complete') =="; exit "$fail"
