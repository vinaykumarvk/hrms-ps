#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-18 verdict — cites the tranche-17 verdict + coverage delta,
...
V=docs/spec/ph-31-verdict.md
echo "== PH-18D exit-criteria (tranche-18 verdict; human gate) =="
...
must "verdict chains from tranche-17 verdict::ph-30-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS02 PS05 PS07; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-31D met (await HUMAN gate review)' || echo 'RED - PH-31D not complete') =="; exit "$fail"
