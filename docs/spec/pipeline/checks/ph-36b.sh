...
V=docs/spec/ph-36-verdict.md
echo "== PH-36B exit-criteria (tranche-23 verdict; human gate) =="
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-36B met (await HUMAN gate review)' || echo 'RED - PH-36B not complete') =="; exit "$fail"
