#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-14 verdict — cites the tranche-13 verdict + coverage delta,
...
V=docs/spec/ph-27-verdict.md
echo "== PH-18D exit-criteria (tranche-14 verdict; human gate) =="
...
must "verdict chains from tranche-13 verdict::ph-26-verdict" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-27D met (await HUMAN gate review)' || echo 'RED - PH-27D not complete') =="; exit "$fail"
