#!/usr/bin/env bash
# PH-60B oracle (human gate): honest tranche-47 verdict — cites the tranche-46 verdict + coverage delta,
# names the coverage ratchet (543->547 / 41.3%) and the PS03 attendance-policy route exposure, remaining-gaps,
...
V=docs/spec/ph-60-verdict.md
echo "== PH-60B exit-criteria (tranche-47 verdict; human gate) =="
...
must "verdict chains from tranche-46 verdict::ph-59-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
must "verdict names the coverage ratchet::547|41.3%|attendance|policy" "$V"
must "verdict carries a PS03 row::PS03" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-60B met (await HUMAN gate review)' || echo 'RED - PH-60B not complete') =="; exit "$fail"
