#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-15 verdict — cites the tranche-14 verdict + coverage delta,
...
V=docs/spec/ph-28-verdict.md
echo "== PH-18D exit-criteria (tranche-15 verdict; human gate) =="
...
must "verdict chains from tranche-14 verdict::ph-27-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS05 PS09 PS13; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-28D met (await HUMAN gate review)' || echo 'RED - PH-28D not complete') =="; exit "$fail"
