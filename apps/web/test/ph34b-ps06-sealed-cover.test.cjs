...
test("PH-34B sealed-cover review is a real controlled form using the client with canonical states", () => {
  for (const m of ["<form", "onSubmit={handleSubmit}", "event.preventDefault()", "useState", "listSealedCovers", "releaseSealedCover", "crypto.randomUUID()"]) assert.equal(comp.includes(m), true, `missing ${m}`);
