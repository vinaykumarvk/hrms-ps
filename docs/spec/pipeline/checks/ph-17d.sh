#!/usr/bin/env bash
# PH-17D oracle (human gate): honest tranche-4 verdict — cites the tranche-3 verdict + coverage delta,
...
V=docs/spec/ph-17-verdict.md
echo "== PH-17D exit-criteria (tranche-4 verdict; human gate) =="
...
must "verdict chains from tranche-3 verdict::ph-16-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS02 PS03 PS09; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-17D met (await HUMAN gate review)' || echo 'RED - PH-17D not complete') =="; exit "$fail"
