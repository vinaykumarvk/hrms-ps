#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-19 verdict — cites the tranche-18 verdict + coverage delta,
...
V=docs/spec/ph-32-verdict.md
echo "== PH-18D exit-criteria (tranche-19 verdict; human gate) =="
...
must "verdict chains from tranche-18 verdict::ph-31-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS06 PS12 PS13; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-32D met (await HUMAN gate review)' || echo 'RED - PH-32D not complete') =="; exit "$fail"
