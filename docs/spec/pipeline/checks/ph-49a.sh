...
if [ -d node_modules ]; then
  ...
  awk "BEGIN{exit !($impl>=475)}" && grn "coverage ratcheted up (implemented $impl >= 475)" || red "coverage did not advance ($impl < 475)"
  awk "BEGIN{exit !($pct>=35.9)}" && grn "coverage >= 35.9% ($pct%)" || red "coverage below 35.9% ($pct%)"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-49A met' || echo 'RED - PH-49A not complete') =="; exit "$fail"
