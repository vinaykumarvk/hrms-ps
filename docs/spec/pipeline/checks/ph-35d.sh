#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-22 verdict — cites the tranche-21 verdict + coverage delta,
...
V=docs/spec/ph-35-verdict.md
echo "== PH-18D exit-criteria (tranche-22 verdict; human gate) =="
...
must "verdict chains from tranche-21 verdict::ph-34-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS01 PS06 PS14; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-35D met (await HUMAN gate review)' || echo 'RED - PH-35D not complete') =="; exit "$fail"
