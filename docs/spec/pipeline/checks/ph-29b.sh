...
have "$R" 'registerPda' "handler calls the backing service (registerPda)"
have "$T"/ph29b-*.test.cjs 'api.dispatch|createFoundationApi' "API test dispatches through the kernel"
have "$T"/ph29b-*.test.cjs '/api/v1/pension/pdas' "API test exercises the route"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-29B met' || echo 'RED - PH-29B not complete') =="; exit "$fail"
