#!/usr/bin/env bash
# PH-43B oracle (human gate): honest tranche-30 verdict — cites the tranche-29 verdict + coverage delta,
# names the coverage ratchet (430->436 / 33%) and the PS14 analytics-engine route exposure, remaining-gaps,
...
V=docs/spec/ph-43-verdict.md
echo "== PH-43B exit-criteria (tranche-30 verdict; human gate) =="
...
must "verdict chains from tranche-29 verdict::ph-42-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
must "verdict names the coverage ratchet::436|33%|analytics|KPI" "$V"
must "verdict carries a PS14 row::PS14" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-43B met (await HUMAN gate review)' || echo 'RED - PH-43B not complete') =="; exit "$fail"
