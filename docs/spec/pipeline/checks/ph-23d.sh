#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-10 verdict — cites the tranche-9 verdict + coverage delta,
...
V=docs/spec/ph-23-verdict.md
echo "== PH-18D exit-criteria (tranche-10 verdict; human gate) =="
...
must "verdict chains from tranche-9 verdict::ph-22-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS01 PS04 PS11; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-23D met (await HUMAN gate review)' || echo 'RED - PH-23D not complete') =="; exit "$fail"
