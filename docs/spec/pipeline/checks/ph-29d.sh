#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-16 verdict — cites the tranche-15 verdict + coverage delta,
...
V=docs/spec/ph-29-verdict.md
echo "== PH-18D exit-criteria (tranche-16 verdict; human gate) =="
...
must "verdict chains from tranche-15 verdict::ph-28-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS10 PS11 PS14; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-29D met (await HUMAN gate review)' || echo 'RED - PH-29D not complete') =="; exit "$fail"
