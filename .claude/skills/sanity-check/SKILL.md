---
name: sanity-check
description: Post-fix sanity check after parallel review remediation. Validates builds, tests, cross-cutting regressions, and re-verifies critical findings from all four review domains (UI, quality, security, infra). Produces a consolidated pass/fail verdict. If a repository-scoped project skill (for example a `<project>-*` skill committed in that repo's .claude/skills/) covers this area, its commands, ports, thresholds, and policies override the generic guidance here.
argument-hint: "[scope]"
---
...
If the user specifies a scope (e.g., `/sanity-check apps/web`), limit file-level checks to that target. Otherwise check the entire monorepo.
...
This playbook is generic. Discover the project's actual build/test commands from `project.config.yaml` (`commands:` section) if present, otherwise from the root `package.json` `scripts` block (and workspace `package.json` files). Never assume a build target exists — enumerate the scripts first, then run the ones that apply.
...
Read the build commands from `project.config.yaml` (`commands:` section) or the root `package.json` `scripts` block — do not guess target names:

```bash
# Discover build scripts
cat package.json | python3 -c "import json,sys; scripts=json.load(sys.stdin).get('scripts',{}); [print(f'  {k}: {v}') for k,v in scripts.items() if 'build' in k.lower()]"
```

If an aggregate script (e.g. `build:all`) exists, run it. Otherwise run each discovered build script individually, in dependency order (shared packages first, then apps that consume them).
...
**Troubleshooting:** A bundler build (`vite build`, esbuild, etc.) is NOT a typecheck — esbuild strips types without checking them. The typecheck authority is the project's real `tsc --noEmit` script (usually `npm run typecheck`); never substitute a bundler build for it. If `tsc` fails on `import.meta.env` or path aliases, fix the tsconfig/`vite-env.d.ts` setup — do not fall back to the bundler. If a shared package build fails, check for circular deps with `npx madge --circular`.

**Gate: All builds must pass. Any build failure = FAIL verdict.**

**Next Step on Failure:** If a package build fails: check the build error, fix the source in the failing package, rebuild that package, then continue with downstream builds. Packages must be rebuilt in dependency order (shared packages first, then the apps that consume them).

## Phase 2: Test Verification

Run all available test suites. Discover them the same way as build scripts — from `project.config.yaml` `commands:` or `package.json` `scripts` (keys containing `test`):

```bash
cat package.json | python3 -c "import json,sys; scripts=json.load(sys.stdin).get('scripts',{}); [print(f'  {k}: {v}') for k,v in scripts.items() if 'test' in k.lower()]"
```
...
```bash
# Check for broken imports across all UI apps — run tsc against each app's tsconfig
for cfg in apps/*/tsconfig.json; do echo "== $cfg =="; npx tsc --noEmit --project "$cfg" 2>&1 | tail -20; done
```

Note: `tsc --noEmit` is the typecheck authority. Never treat a passing `vite build` (or any esbuild-based bundler build) as evidence of type safety — it strips types without checking them. If an app has `import.meta.env` or path-alias errors under `tsc`, fix its `vite-env.d.ts`/tsconfig instead of substituting the bundler build.
...
If the monorepo has shared packages (e.g. `packages/shared/`), verify that changes to their exported modules didn't break consumers:
- Check all apps that import from the shared package still build
...
```bash
# i18n anti-pattern: string-only t() passed where the project's bilingual/stacked
# label component is required (only if the project has such a rule)
rg -n 'label=\{t\(' apps/*/src --glob '*.tsx'
...
- `label={t(` matches: Replace with the project's bilingual/stacked label component (e.g. `label={<Bilingual tKey="key" />}` in projects that define one)
