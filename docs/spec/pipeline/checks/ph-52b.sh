#!/usr/bin/env bash
# PH-52B oracle (human gate): honest tranche-39 verdict — cites the tranche-38 verdict + coverage delta,
# names the coverage ratchet (489->495 / 37.4%) and the PS06 sanctioned-post route exposure, remaining-gaps,
...
V=docs/spec/ph-52-verdict.md
echo "== PH-52B exit-criteria (tranche-39 verdict; human gate) =="
...
must "verdict chains from tranche-38 verdict::ph-51-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
must "verdict names the coverage ratchet::495|37.4%|sanctioned|establishment" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-52B met (await HUMAN gate review)' || echo 'RED - PH-52B not complete') =="; exit "$fail"
