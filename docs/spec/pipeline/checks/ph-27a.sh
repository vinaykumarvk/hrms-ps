#!/usr/bin/env bash
# PH-27A oracle: PS13 DPDP DSR console UI — a real controlled web surface using the injected client
# with canonical loading/error/empty states; the client carries the adjudicateDsr method; web suites green.
set -uo pipefail
cd "$(git -C "$(dirname "$0")" rev-parse --show-toplevel 2>/dev/null || echo /Users/n15318/hrms)"
fail=0; red(){ echo "  RED  $*"; fail=1; }; grn(){ echo "  ok   $*"; }
have(){ grep -qE "$2" "$1" 2>/dev/null && grn "$3" || red "$3"; }
COMP="apps/web/src/modules/ps13/DataSubjectRequestConsole.tsx"; CLIENT="apps/web/src/api/hrmsClient.ts"; FIX="apps/web/src/api/fixtureHrmsClient.ts"; APP="apps/web/src/App.tsx"
echo "== PH-27A exit-criteria (PS13 DPDP DSR console UI) =="
...
have "$COMP" 'adjudicateDsr' "component calls client.adjudicateDsr"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-27A met' || echo 'RED - PH-27A not complete') =="; exit "$fail"
