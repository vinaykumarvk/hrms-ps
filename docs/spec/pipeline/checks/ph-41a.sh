...
if [ -d node_modules ]; then
  ...
  awk "BEGIN{exit !($impl>=421)}" && grn "coverage ratcheted up (implemented $impl >= 421)" || red "coverage did not advance ($impl < 421)"
  awk "BEGIN{exit !($pct>=31.8)}" && grn "coverage >= 31.8% ($pct%)" || red "coverage below 31.8% ($pct%)"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-41A met' || echo 'RED - PH-41A not complete') =="; exit "$fail"
