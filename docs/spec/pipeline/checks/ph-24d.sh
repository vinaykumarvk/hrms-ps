#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-11 verdict — cites the tranche-10 verdict + coverage delta,
...
V=docs/spec/ph-24-verdict.md
echo "== PH-18D exit-criteria (tranche-11 verdict; human gate) =="
...
must "verdict chains from tranche-10 verdict::ph-23-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS06 PS11 PS12; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-24D met (await HUMAN gate review)' || echo 'RED - PH-24D not complete') =="; exit "$fail"
