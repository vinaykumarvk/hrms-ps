---
name: infra-review
description: End-to-end architecture and infrastructure review covering system boundaries, performance, reliability, observability, Dockerfiles, CI/CD pipelines, environment configuration, and deployment readiness. Produces a prioritized improvement plan with a readiness verdict. If a repository-scoped project skill (for example a `<project>-*` skill committed in that repo's .claude/skills/) covers this area, its commands, ports, thresholds, and policies override the generic guidance here.
