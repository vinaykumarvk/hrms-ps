#!/usr/bin/env bash
# PH-55B oracle (human gate): honest tranche-42 verdict — cites the tranche-41 verdict + coverage delta,
# names the coverage ratchet (513->519 / 39.2%) and the PS01 governed write-port route exposure, remaining-
...
V=docs/spec/ph-55-verdict.md
echo "== PH-55B exit-criteria (tranche-42 verdict; human gate) =="
...
must "verdict chains from tranche-41 verdict::ph-54-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
must "verdict names the coverage ratchet::519|39.2%|write.port|identity" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-55B met (await HUMAN gate review)' || echo 'RED - PH-55B not complete') =="; exit "$fail"
