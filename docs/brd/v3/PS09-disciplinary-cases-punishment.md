# Employee Disciplinary Cases and Punishment Management — PrimeSoft HRMS Module BRD (PS09, v3.0 · platform-grounded)

**Module code:** PS09 (PrimeSoft HRMS · `SHARED_FOUNDATION` M09-DCP; re-keyed per `MODULE_RECONCILIATION.md` §B)
**Program:** PrimeSoft HRMS — a public-sector configuration and extension of the **PrimeSoft HRMS platform** (Master BRD v2.1 · Product Vision v2.6 · Platform Spec v1.6 · RBAC Design v1.7 · Foundation FS v1.6).
**Document version:** v3.0 (platform-re-grounded; supersedes v2.0 / v1.0).
**Relationship:** **NET-NEW (enterprise-specific)** statutory due-process engine — PrimeSoft has no disciplinary module (`MODULE_RECONCILIATION.md` §A, row PS09; §D). The due-process business logic is authored here but **runs on platform engines**: P01 (workflow/SLA), P05 (audit/immutability), P06 (migration). It posts penalty/exoneration to the **PS12 SR ledger** (P05 substrate) and flows effects by reference to **PS06** (seniority/promotion), **PS10** (recovery/subsistence) and **PS11** (pension).
**Status:** Baseline for build — legality layer hardened per Council Report; re-anchored onto PrimeSoft platform contracts (v3).
...
**Platform contract (binding, supersedes the invented `SHARED_FOUNDATION.md`):** This BRD consumes — and does **not** redefine — the PrimeSoft platform contracts in [`PLATFORM_FOUNDATION.md`](../PLATFORM_FOUNDATION.md) and the mapping/overrides in [`MODULE_RECONCILIATION.md`](../MODULE_RECONCILIATION.md). Workflow (P01), RBAC enforcement (P02), audit (P05), notifications (X.2), background jobs (X.1), configured flows/forms (W.1/W.2/W.3), the API conventions, the `VAL-*`/`MSG-*`/`ERR-*` catalogues, and the NFR baseline are referenced **by id**, never re-authored. Every entity carries `tenant_id` (and, where entity-scoped, `entity_id`); see `## Alignment with PrimeSoft Platform` and `## Amendments (v2 → v3: platform re-grounding)`.
**Revision basis:** Adversarial Council Report `/Users/n15318/hrms/docs/evaluation/M09-disciplinary-cases-punishment-council.md` — all 24 Adopted Improvements and every High/Critical Risk Register item incorporated (see §1.6 Amendments), now re-grounded on the platform.
...
The Employee Disciplinary Cases and Punishment Management module (**PS09-DCP**) digitises the **end-to-end disciplinary lifecycle** of a public-sector / enterprise employer, from the first receipt of a complaint or detection of misconduct through preliminary inquiry, optional suspension, framing of articles of charge, departmental inquiry, penalty imposition, and statutory appeal / revision / review — concluding with an immutable posting of the outcome (punishment **or** exoneration) into the Digital Service Register (PS12).
...
PS09-DCP converts this risk-laden manual process into a **controlled, time-bound, fully audited, two-layer workflow** — an **invariant natural-justice kernel** (charge → genuine opportunity to defend → reasoned finding on evidence → proportionate penalty within what was proposed, by a competent authority → independent appeal) wrapped in a **configurable jurisdiction overlay** (`procedure_template`: required consultations, authority-competence matrix, statutory timelines, valid service modes, appeal limitation, dispense-with-inquiry conditions). Seed reference data ships the CCS(CCA) regime as the default; a corporate code of conduct or a foreign-subsidiary regime is simply a different template.
...
| O5 | Propagate outcomes to dependent modules accurately and idempotently | Penalty/exoneration posted exactly-once to PS12 SR; effects flowed to PS06/PS10/PS11; 0 duplicate downstream effects |
...
Out of scope (owned elsewhere, integrated here): the SR ledger itself (PS12), payroll execution of recoveries/subsistence (PS10), pension re-computation engine (PS11), seniority list recomputation (PS06), the physical document store (PS13), identity/SSO and the DSC/eSign trust service (platform), and external court/police case systems (reference-linked only).
...
|---|---|---|---|
| AI-1 / R1 | `authority_competence` reference entity + DI-13 penalty-competence guard; error `ERR-PS09-AUTHORITY-NOT-COMPETENT` (Art. 311(1)) | Critical | New entity **E23 `authority_competence`** (§5.2); **DI-13** (§5.6); **FR-PS09-018** (competence matrix + finalise-time guard); error code §10.3; state guard §12.4 |
| AI-2 / R2 | `case_consultations` entity + DI-14; finalise blocked until required consultations closed; error `ERR-PS09-CONSULTATION-PENDING` (UPSC/CVC) | Critical | New entity **E24 `case_consultations`** (§5.2); **DI-14** (§5.6); **FR-PS09-019**; error §10.3; gate in FR-011 finalise |
| AI-3 / R3 | Lawful "dispense-with-inquiry" path (`inquiry_dispensations`, reason-coded) — Art. 311(2) provisos | High | New entity **E25 `inquiry_dispensations`** (§5.2); **DI-23** (§5.6); **FR-PS09-020**; satisfies DI-3 major branch via documented exception |
| AI-4 / R4 | Document/witness list supplied + inspection afforded right; inquiry cannot proceed to evidence without it | High | Fields on `inquiry_proceedings` (`list_supplied_date`, `inspection_afforded_date`); **DI-24** (§5.6); amended **FR-PS09-007**; error `ERR-PS09-INSPECTION-NOT-AFFORDED` |
| AI-5 / R5 | Separate `legal_service` record from informational `notifications`; valid `service_mode` per template; EMAIL non-statutory by default | High | New entity **E26 `legal_service_records`** (§5.2); **DI-15** (§5.6); **FR-PS09-021**; valid modes carried on `procedure_templates` |
| AI-6 / R6 | Subsistence: `non_employment_certificate_received` gate; deemed-review / auto-flag on 90-day charge-memo lapse (Ajay Kumar Choudhary) | High | Fields on `suspensions` (`non_employment_certificate_received`, `nec_received_date`, `charge_memo_due_date`, `deemed_review_flag`); **DI-16** (§5.6); amended **FR-PS09-003** |
| AI-7 / R7 | Common/joint proceedings — `case_respondents` (1..N) sharing inquiry/evidence; per-respondent article findings | High | New entity **E27 `case_respondents`** (§5.2); **DI-25** (§5.6); **FR-PS09-022**; `charge_articles` finding moved to per-respondent grain |
| AI-8 / R8 | Two-layer model: `procedure_template` overlay (consultations, competence ref, timelines, service modes, limitation, dispensation); kernel invariant | High | New entity **E22 `procedure_templates`** (§5.2); **DI-17** (§5.6); **FR-PS09-017**; `procedure_template_id` on `disciplinary_cases` |
...
| AI-10 / R10 | `sla_pause_events` ledger + recompute `expected_closure_date`/`sla_target_at` on stay/remit/condonation/consultation/criminal-stay | Medium | New entity **E28 `sla_pause_events`** (§5.2); **DI-18** (§5.6); **FR-PS09-024**; amended FR-016 |
| AI-11 / R11 | Idempotency-key lifecycle (mint, dedup store, TTL); double-finalise / double post-to-SR ⇒ exactly one effect | Medium | New entity **E30 `idempotency_keys`** (§5.2); **DI-19** (§5.6); §10.1 lifecycle; amended **FR-PS09-013**; error `ERR-PS09-IDEMPOTENCY-CONFLICT` |
| AI-12 / R12 | Mandatory `proportionality_reasoning` on `penalty_orders` + analytics outlier flag | Medium | Field on **E16** (§5.2); **DI-20** (§5.6); amended **FR-PS09-011** + **FR-PS09-016**; error `ERR-PS09-PROPORTIONALITY-REASONING-REQUIRED` |
| AI-13 / R13 | Mid-proceeding jurisdiction transfer (re-resolve DA), sealed-cover promotion freeze, Rule 9 four-year-bar + sanction for retirees | Medium | `case_jurisdiction_transfer` timeline event + fields; **DI-27** (§5.6); **FR-PS09-026**; sealed-cover flag feeds PS06 |
| AI-14 / R14 | Abatement terminal handling: `order_type=ABATED`, `case_status=ABATED`; auto-stop SLA + suppress effects on death | Low/Med | Enum extensions (§5.5); **DI-26** (§5.6); **FR-PS09-028**; state §12.1 |
| AI-15 / R15 | Court-grade hash-chained `audit_log`/`case_timeline_events` (prev-hash linkage) + verify endpoint | Medium | `row_hash`/`prev_hash` on **E19** (§5.2); **DI-21** (§5.6); **FR-PS09-027**; verify API §10 |
| AI-16 / R16 | POSH/ICC procedure template for HARASSMENT — Internal Committee composition/timelines feeding penalty stage | High | `procedure_templates` ICC variant; `inquiry_appointments` ICC roles; **FR-PS09-023**; error `ERR-PS09-ICC-PROCEDURE-REQUIRED` |
| AI-17 / R17 | DSC/eSign with trusted timestamp + bound signatory identity on charge-sheets, reports, notices, orders | Medium | Signature fields on `penalty_orders`/`legal_service_records`/`case_documents`; **DI-28** (§5.6); **FR-PS09-021**; error `ERR-PS09-SIGNATURE-REQUIRED` |
| AI-18 / R18 | Inquiry `STAYED` sub-state + auto SLA pause when parallel criminal trial active | Medium | `inquiry_status=STAYED` (§5.5); fields on `inquiry_proceedings`; amended **FR-PS09-007**; pause reason `CRIMINAL_STAY` (FR-024) |
| AI-19 / R19 | Charged-Officer Portal → rights-and-deadlines surface (entitlements + limitation countdown) | Low | Amended **FR-PS09-005** UI; §7.1 portal redesign; §7.3 rights panel |
| AI-20 / R20 | Recovery-penalty caps (≤1/3 pay, instalments, not beyond retirement except DCRG) validated pre-emit | Medium | **DI-22** (§5.6); amended **FR-PS09-011**; error `ERR-PS09-RECOVERY-CAP-EXCEEDED` |
| AI-21 | Personal-hearing record for show-cause and appeal stages | — | New entity **E29 `personal_hearings`** (§5.2); **DI-29** (§5.6); **FR-PS09-025**; amended FR-010/FR-012 |
...
| AI-24 | Disclose adverse PI material when relied upon as evidence; served/relied-upon flag | — | **DI-9** extended (§5.6); amended **FR-PS09-002**/**FR-PS09-007**; `relied_upon`/`disclosed` flags on `inquiry_exhibits` |
...
| Area | Feature | FR(s) |
|------|---------|-------|
| Intake | Complaint / source-of-misconduct registration & triage | FR-PS09-001 |
| Fact-finding | Preliminary (fact-finding) inquiry (+ adverse-material disclosure) | FR-PS09-002 |
| Interim action | Suspension (parallel status), revocation, subsistence + NEC gate, deemed review | FR-PS09-003 |
| Charge | Charge-sheet / articles of charge / statement of imputations | FR-PS09-004 |
| Defence | Employee's written statement of defence; admission/denial; rights surface | FR-PS09-005 |
| Inquiry setup | Appointment of Inquiry Officer & Presenting Officer | FR-PS09-006 |
| Inquiry conduct | Hearings, witnesses, exhibits, daily order sheets, ex-parte, inspection right, STAY | FR-PS09-007 |
| Inquiry close | Inquiry report & findings (proved / not proved) | FR-PS09-008 |
| DA consideration | Disagreement memo & disciplinary authority decision | FR-PS09-009 |
| Natural justice | Show-cause on proposed penalty (major penalty / enhancement) | FR-PS09-010 |
| Outcome | Penalty order (minor/major) & exoneration; proportionality; recovery caps | FR-PS09-011 |
| Remedies | Appeal / revision / review | FR-PS09-012 |
| Propagation | SR posting & downstream effects (PS06/PS10/PS11/PS12) — idempotent | FR-PS09-013 |
| Records | Document evidence vault (link to PS13) | FR-PS09-014 |
| Oversight | Integrity register & vigilance clearance status | FR-PS09-015 |
| Governance | SLA / statutory-timeline tracking & case analytics | FR-PS09-016 |
| Configuration | Procedure-template & jurisdiction overlay (two-layer model) | FR-PS09-017 |
| Legality | Authority-competence matrix & finalise-time competence guard | FR-PS09-018 |
| Legality | Statutory consultation management (UPSC/CVC/ICC/legal) | FR-PS09-019 |
| Legality | Dispense-with-inquiry (Article 311(2) provisos) | FR-PS09-020 |
| Legality | Legal service of documents & digital signing (DSC/eSign) | FR-PS09-021 |
| Scale | Common/joint (multi-respondent) proceedings | FR-PS09-022 |
| Compliance | POSH / Internal Committee procedure | FR-PS09-023 |
| Governance | SLA pause/resume & clock management | FR-PS09-024 |
| Natural justice | Personal hearing record (show-cause / appeal) | FR-PS09-025 |
| Lifecycle | Mid-proceeding jurisdiction transfer, sealed cover & retiree proceedings | FR-PS09-026 |
| Integrity | Court-grade hash-chained audit & verification | FR-PS09-027 |
| Lifecycle | Abatement on death / terminal handling | FR-PS09-028 |

### 2.2 Common Capabilities (inherited, applied module-wide)

- **Maker-checker / workflow** runs on the **platform P01 WorkflowEngine** (`workflows` / `workflow_instances` / `workflow_actions`; **not** an invented `workflow_actions` table) for every stage transition that changes statutory state. The disciplinary stages (preliminary inquiry → charge-sheet → inquiry → DA consideration/consultation → show-cause → penalty → appeal) are **configured P01/W.1 flow definitions** whose subjects are the PS09 case/charge/inquiry/penalty/appeal entities; PS09 calls `WorkflowEngine.startInstance/advance/approve/reject/sendBack/delegate/cancel` (each idempotent) and does **not** build a parallel engine.
- **Segregation of duties (extended):** maker ≠ checker is **enforced by P01/P02** (multi-role INTERSECTION, no self-approval), not re-coded; the Disciplinary Authority (DA), Inquiry Officer (IO), Presenting Officer (PO), and any witness must be **mutually distinct** persons; **the DA may not also be the complainant or a witness** (DI-2, nemo judex in causa sua).
- **Two-layer procedure model:** an invariant natural-justice **kernel** plus a configurable **`procedure_template`** overlay; the kernel is never parameterised away (DI-17). The overlay is realised as **W.1 flow + W.2 form** configuration on P01; PS09 builds no generic BPM/rules designer.
- **SLA & statutory-timeline runtime** is the **P01 per-stage SLA timer + escalation** runtime (breach → escalation event → X.2 notification + P05 audit); lawful clock pause/resume is driven by **`JOB-PS09-SLA`** / **`JOB-PS09-TIMELINE`** background jobs registered against the **X.1** runner and indexed in Foundation §4 (pause/resume recompute, DI-18).
- **Competence & consultation gating:** finalise is blocked unless the signing authority is competent for the penalty class (DI-13) and all template-required consultations are closed (DI-14) — enforced as P01 stage-entry guards.
- **Tamper-evident audit on the platform P05 substrate:** the **P05 dual log** (`audit_log` + `security_audit_log`) captures every INSERT/UPDATE/soft-DELETE via **DB trigger** (immutable, PII-masked, ≥ 7-yr retention) — no module-defined `audit_log`. Statutory-grade tamper-evidence tracks **OPEN-PLAT-03** (periodic hash-chaining of audit partitions, chain head to WORM); the PS09 `case_timeline_events` ledger is a domain SLA/stage ledger that **rides on** this substrate rather than inventing a parallel cryptographic chain (DI-21 re-grounded).
- **Confidentiality / sealed-cover / field masking via P02:** the PII Protection Ceiling, confidential-roles visibility and **field mask on serialization** are owned by RBAC v1.7 and enforced by `Authorization.check` (an over-broad query cannot leak a masked field); PS09 declares confidential-source/sealed-cover fields and roles, it does not re-implement masking (DI-9, §3.3).
- **Legal service ≠ notification:** statutory service is recorded in `legal_service_records` with proof; the platform `notifications` ledger (X.2) is informational only (DI-15).
- **Digital signing:** statutory artefacts carry DSC/eSign signature metadata (DI-28).
- **Document handling** via **PS13** document vault (the enterprise document module running on PrimeSoft M11 Document Management) — versioned, encrypted, access-controlled `documents` objects referenced by metadata.
- **Notifications** via **X.2** infrastructure: `IN_APP` + `EMAIL` fire in parallel; **EMAIL for statutory notices is mandatory and non-suppressible** (Platform §X.2; BRD §9.9). Templates referenced by `MSG-PS09-*` id (Foundation §5), never inlined.
- **Idempotency:** unsafe POSTs that create a transaction carry an **`Idempotency-Key`** header (platform convention; 24h replay window for the header, with a domain `idempotency_keys` ledger for propagating posts, DI-19).
- **Pagination:** **cursor pagination only** — `?limit=` (default 25, max 100) + `cursor=`, response carries `next_cursor` (Foundation §1); offset/page paging is not used.
- **Multi-tenancy:** every PS09 entity carries **`tenant_id`** (and `entity_id` where entity-scoped); scoping is enforced at the **data layer** and a query without a resolvable tenant scope is rejected, not defaulted to "all" (Platform §0.1).
- **Localisation:** UTC storage, `DD-MMM-YYYY` display, INR money formatting.
- **RBAC + five-dimension row-level scoping** (RBAC v1.7), extended in §3 with field-level confidentiality controls enforced by P02.

### 2.3 Boundaries & integration points

| Boundary | Direction | Contract |
|----------|-----------|----------|
| PS01-EPM (employees) | read | Charged employee(s), complainant, IO/PO identity, designation, cadre, org_unit, employment_status |
| PS05-TRJ (transfers) | read (event) | Mid-proceeding transfer triggers jurisdiction re-resolution (FR-026) |
| PS06-PPP | write (event) + read | Reduction in rank, withholding of promotion, seniority re-fixation; **sealed-cover** promotion freeze |
| PS10-PAY | write (event) | Suspension → subsistence (NEC-gated); recovery (capped); pay reduction; promotion/increment withholding |
| PS11-PEN | write (event) | Pension cut/withholding; effect of removal/dismissal/CR; retiree-proceeding flags; Rule 9 |
| PS12-SR (service_register_events) | write (append-only, idempotent) | Suspension, penalty, exoneration, appeal outcome, abatement events |
| PS13-DMS | read/write | Evidence vault objects, charge-sheet/order PDFs (DSC-signed) |
| PS14-DAS | read | KPI feed incl. proportionality-outlier dashboard |
| Platform DSC/eSign trust service | call (X.3) | Sign + verify trusted timestamp and signatory identity binding; credentials via `integration_credentials` (P04), outbound calls on X.3 |
| Platform **P01** WorkflowEngine | call | Due-process stages run as configured P01/W.1 flows; SLA/escalation runtime |
| Platform **P02** Authorization.check | call | Per-request RBAC + scope + field-mask resolution (no permission logic re-implemented) |
| Platform **P05** Audit & PS12 SR substrate | write/read | DB-trigger dual-log audit; PS12 SR ledger append (P05 immutability substrate) |
| Platform **X.1 / X.2 / P04 / P06** | use | `JOB-PS09-*` jobs; X.2 notifications; integration credentials; P06 legacy migration |

### 2.4 Explicit non-goals

- PS09 does **not** compute payroll, pension, or seniority figures; it emits **events** that the owning module executes.
- PS09 does **not** store document binaries; it stores references to PS13 objects.
- PS09 does **not** operate the DSC/eSign Certifying Authority; it calls the platform trust service and stores returned signature metadata.
...
- PS09 is **not** a generic BPM designer; configurability is **capped** at the named overlay items in `procedure_templates` (DI-17).

---

## 3. Roles & Permissions

### 3.1 Module roles (mapped to RBAC v1.7 as ADDITIONS — no parallel scheme)

> **Platform re-grounding (v3).** Enterprise statutory actors are expressed as **new roles + capability flags ADDED to the RBAC v1.7 taxonomy** (`PLATFORM_FOUNDATION.md` §6.6; `MODULE_RECONCILIATION.md` §C), registered in RBAC §4.3/§2.2 with grant authority and audit logging. **Segregation of duties (maker ≠ checker, no self-approval, multi-role INTERSECTION) is enforced by P01/P02 — not re-implemented here.** Enforcement (deny-by-default → role grant → INTERSECTION → entitlement → capability flag → PII Ceiling → scope filter → field mask) is owned by P02; this BRD only declares the roles, capability flags and SoD constraints.
>
> | PS09 statutory actor | Express in RBAC v1.7 as | SoD / capability flag |
> |---|---|---|
> | **Disciplinary Authority (DA)** | new sanctioning role (P01 approver in due-process flow) | cannot self-approve; ≠ complainant/witness/IO/PO (DI-2); MFA-enforced like HR/Org Admin (Platform §3.1) |
> | **Inquiry Officer (IO)** | new role + case-scoped read entitlement (RBAC §3.2 time-bound entitlement pattern) | distinct from DA/PO/witness; case-scoped data access |
> | **Presenting Officer (PO)** | new role / capability flag | distinct from DA/IO; presents department case |
> | **Appellate / Reviewing Authority** | new role (P01 approver, appeal flow) | ≠ DA; competence-verified (DI-13) |
> | **ICC Presiding / Member / External Member** | new POSH roles + external-member capability flag | replaces IO route for HARASSMENT; external member mandatory |
> | **Vigilance Officer / Consulting-Authority Liaison / Defence Assistant** | new entity-scoped roles + capability flags | confidential-source visibility (P02 PII ceiling) |
> | **Disciplinary Case Manager / HR-DCP Admin** | map to **HR Admin** (RBAC §2.2) + PS09 module-admin entitlement | operational, non-deciding |
> | **Auditor (read-only)** | map to **Org-Admin audit access + read-only entitlement** (P05 query) | no write; runs P05/OPEN-PLAT-03 chain verify |
> | **System Administrator** | map to **Org Admin / Platform Super Admin** (RBAC §2.1) | configures `procedure_templates`/`authority_competence`; no transactional self-approval |
>
> New flags are registered in RBAC §4.3 via the working-group process; `tenant_id`/`entity_id` scoping applies to every role.
...
| Vigilance Officer | Screens complaints, maintains integrity register & vigilance clearance | PS09 |
...
| Inquiry Officer (IO) | Conducts the departmental inquiry impartially | Shared (PS09-specific) |
| Presenting Officer (PO) | Presents the case on behalf of the department before the IO | PS09 |
| Internal Committee (ICC) Member / Presiding Officer / External Member | POSH Internal Committee for HARASSMENT cases (replaces IO route) | PS09 (POSH) |
| Defence Assistant (DA-Asst) | Person assisting the charged officer | PS09 |
| Disciplinary Case Manager / HR-DCP Admin | Operates the case workbench, drafts artefacts, manages SLAs | Shared (HR Admin) |
| Consulting Authority Liaison | Records UPSC/CVC/legal consultation references and advice | PS09 |
| Appellate Authority | Decides appeals | PS09 |
| Reviewing / Revising Authority | Exercises suo-motu review/revision | PS09 |
...
## 4. PrimeSoft Platform Foundation (re-grounded — supersedes the invented `SHARED_FOUNDATION.md`)

This module **consumes** the PrimeSoft platform contracts in [`PLATFORM_FOUNDATION.md`](../PLATFORM_FOUNDATION.md) and the overrides in [`MODULE_RECONCILIATION.md`](../MODULE_RECONCILIATION.md) §C. The invented `SHARED_FOUNDATION.md` conventions are **overridden** as follows:

- **Architecture & deployment:** logical architecture and NFR are owned by the platform (Vision Part 2 / Platform Spec); the physical stack is an engineering choice within that logical architecture, and the deployment target (enterprise cloud / CGG Data Centre) is a Standalone/Group-Company deployment model — **not** re-architected here (`MODULE_RECONCILIATION.md` §C). REST under **`/api/v1`**; PostgreSQL with **data-layer tenant/entity scoping**; encrypted object storage via **PS13** for binaries.
- **Multi-tenancy (was omitted):** every PS09 table carries **`tenant_id`** (non-nullable) and, where entity-scoped, **`entity_id`**; cross-entity reach is an Org-Admin widened scope filter, never a bypass; an unscoped query is rejected, not defaulted to "all" (Platform §0.1).
- **Auth & session (P02-resolved):** bearer-token (JWT) session carries resolved roles + tenant/entity scope; **permissions are resolved per request by `Authorization.check` (P02)** — endpoints never re-implement permission logic. Mechanisms: Google SSO / username-password (hashed) / **MFA enforced for high-privilege statutory roles** (DA, Appellate Authority, Pension/Payroll Officer). Step-up MFA for penalty/appeal decisions; **DSC/eSign** for statutory signing.
- **API conventions (adopted verbatim, Foundation §1):** `/api/v1`; **`Idempotency-Key`** on unsafe transaction-creating POSTs (24h replay); **cursor pagination only** (`?limit=` default 25 / max 100 + `cursor=` → `next_cursor`); `?sort=field:asc|desc`; **`X-Correlation-Id`** on every request/response and written to every audit/log line; effective-dating staged via job, not live.
- **Canonical error envelope (platform):** `{ "error": { "code": "...", "message": "...", "field": "...", "details": { } } }`; the correlation id is the **`X-Correlation-Id` response header**, **not** a body `requestId`.
- **Standard error codes (platform 8-code table, Foundation §1):** `VALIDATION_FAILED (422)`, `UNAUTHENTICATED (401)`, `FORBIDDEN (403)`, `NOT_FOUND (404)`, `CONFLICT (409)`, `PRECONDITION_FAILED (412)`, `RATE_LIMITED (429)`, `INTERNAL (500)`. The invented `VALIDATION_ERROR (400)` / `AUTH_REQUIRED (401)` / `INTERNAL_ERROR (500)` / `UPSTREAM_UNAVAILABLE (503)` are replaced (upstream failures map via X.3 + 500/`ERR-LOADFAIL`). Shared user-facing copy uses `ERR-*` ids; module-specific codes in §10.
- **Validation:** cite the 36-entry `VAL-*` catalogue by id (Foundation §2); author only module-unique `VAL-PS09-*` (e.g. `VAL-PS09-DUEPROCESS` charge→reply→inquiry→penalty sequencing, `VAL-PS09-COMPETENCE`, `VAL-PS09-SERVICE`) and register them in the Foundation index.
- **Audit (P05, not a custom log):** the **dual log** (`audit_log` + `security_audit_log`) is captured by **DB trigger** on every mutation, immutable, PII-masked, ≥ 7-yr retention; reading an audit row is itself audited. Tamper-evidence tracks **OPEN-PLAT-03** hash-chaining (chain head → WORM). PS09 defines **no** `audit_log` of its own.
- **Notifications (X.2):** `IN_APP` + `EMAIL` in parallel; **EMAIL for statutory notices mandatory/non-suppressible** (BRD §9.9); templates by `MSG-PS09-*` id.
- **Background jobs (X.1):** `JOB-PS09-SLA` (stage SLA pre-breach/breach escalation), `JOB-PS09-TIMELINE` (statutory-timeline + pause/resume recompute), `JOB-PS09-EFFDATE` (effective-dated order/penalty effects) registered against the runner and indexed in Foundation §4; runner guarantees idempotency, backoff ×3, `JOB-FAIL`→`MSG-SYS-JOBFAIL`, per-tenant isolation, run audit row.
- **Migration (P06):** legacy disciplinary registers migrate through ETL+V (3 staging dry runs, waves, `migration_runs`, `<enterprise>_source_id` traceability).
- **Security/compliance:** OWASP, TLS 1.2+, encryption at rest, DPDP Act 2023 / DPDPA consent (`consent_records`, `VAL-CONSENT`), statutory retention (never below the statutory floor; right-to-erasure does not apply to statutory disciplinary data).
- **NFR baseline (platform, Vision §2.9 — overrides the invented values):** standard API **p95 < 500 ms @ 300 concurrent**; read-heavy p95 < 300 ms cached / < 1000 ms uncached; writes p95 < 1500 ms; **uptime 99.5%/month**; **RTO < 4 h, RPO < 1 h**; 100% audit mutation capture; **WCAG 2.1 AA**; responsive 375/768/1280 px, touch ≥ 44×44 px; **no hard delete — soft delete only**.

**Reused platform-provided entities (referenced, never redefined):** `employees`, `users`, `org_units`, `designations`/`cadres`/`pay_scales` (PS01 master on PrimeSoft M01), `documents` (PS13 / PrimeSoft M11), `notifications`, `consent_records`, `integration_credentials`, `migration_runs`, the **P05** `audit_log` / `security_audit_log`, and the **P01** `workflows` / `workflow_instances` / `workflow_actions` (the invented `workflow_actions` is replaced by `workflow_actions`). `service_register_events` is **not** a platform primitive — it is the **net-new PS12 SR ledger** running on the P05 substrate, which PS09 **appends to** (DI-10/DI-19).
...
|---|--------|------|-------|---------|:--:|
| E1 | `disciplinary_cases` | Module | PS09 | Master case record (the file) | — |
| E2 | `case_complaints` | Module | PS09 | Source-of-misconduct / complaint intake records | — |
| E3 | `preliminary_inquiries` | Module | PS09 | Fact-finding inquiry before formal charges | — |
| E4 | `suspensions` | Module | PS09 | Suspension / revocation / subsistence allowance (NEC + deemed review) | — |
| E5 | `charge_sheets` | Module | PS09 | Memorandum of charges (articles of charge container) | — |
| E6 | `charge_articles` | Module | PS09 | Individual article of charge + statement of imputation | — |
| E7 | `defence_statements` | Module | PS09 | Charged officer's written statement of defence | — |
| E8 | `inquiry_proceedings` | Module | PS09 | The departmental inquiry instance (+ STAY, inspection) | — |
| E9 | `inquiry_appointments` | Module | PS09 | IO / PO / Defence Assistant / ICC appointments | — |
| E10 | `inquiry_hearings` | Module | PS09 | Daily order sheets / hearing log | — |
| E11 | `inquiry_witnesses` | Module | PS09 | Listed/examined witnesses (prosecution & defence) | — |
| E12 | `inquiry_exhibits` | Module | PS09 | Documentary/material evidence (vault items; relied-upon/disclosed) | — |
| E13 | `inquiry_reports` | Module | PS09 | IO/ICC findings (article-wise per respondent) | — |
| E14 | `disagreement_memos` | Module | PS09 | DA disagreement with IO findings | — |
| E15 | `show_cause_notices` | Module | PS09 | Notice on proposed penalty | — |
| E16 | `penalty_orders` | Module | PS09 | Final order (penalty/exoneration/drop/abated) + proportionality + signature | — |
| E17 | `penalty_items` | Module | PS09 | Individual penalty(ies) imposed (recovery caps) | — |
| E18 | `appeals` | Module | PS09 | Appeal / revision / review applications & decisions | — |
| E19 | `case_timeline_events` | Module | PS09 | SLA-tracked stage events (hash-chained ledger) | — |
| E20 | `vigilance_records` | Module | PS09 | Integrity register & vigilance clearance status | — |
| E21 | `case_documents` | Module | PS09 (link) | Join between case artefacts and PS13 `documents` (+ signature) | — |
| **E22** | `procedure_templates` | Module | PS09 | Jurisdiction overlay: consultations, competence ref, timelines, service modes, limitation, dispensation conditions | ✔ |
| **E23** | `authority_competence` | Module | PS09 | (cadre/level × penalty_class) → empowered authority level matrix | ✔ |
| **E24** | `case_consultations` | Module | PS09 | UPSC/CVC/ICC/legal consultation records gating finalise | ✔ |
| **E25** | `inquiry_dispensations` | Module | PS09 | Lawful dispense-with-inquiry (Art. 311(2) provisos) | ✔ |
| **E26** | `legal_service_records` | Module | PS09 | Statutory legal service (mode, proof, served_by) distinct from notifications | ✔ |
| **E27** | `case_respondents` | Module | PS09 | 1..N charged officers per proceeding (common/joint proceedings) | ✔ |
| **E28** | `sla_pause_events` | Module | PS09 | Clock pause/resume ledger (stay/remit/condonation/consultation/criminal) | ✔ |
| **E29** | `personal_hearings` | Module | PS09 | Personal-hearing record (show-cause / appeal stages) | ✔ |
| **E30** | `idempotency_keys` | Module | PS09 | Dedup store for propagating posts (mint, TTL) | ✔ |
| — | `employees`, `org_units`, `designations`/`cadres`, `service_register_events`, `documents`, `notifications`, `audit_log`, `workflow_*` | Shared | PS01/PS12/PS13/platform | Referenced, not redefined | — |
...
| `non_employment_certificate_received` | BOOLEAN | N | **(v2)** Precondition for subsistence payment to PS10 (AI-6) |
...
| `payroll_event_id` | UUID | Y | Correlation to PS10 |
...
| `side` | ENUM `witness_side` | N | PROSECUTION / DEFENCE |
| `description` | TEXT | N | |
| `document_id` | UUID FK→documents | N | Vault item (PS13) |
...
| `sr_event_id` | UUID FK→service_register_events | Y | PS12 correlation |
...
| `downstream_event_id` | UUID | Y | Correlation to PS06/PS10/PS11 |
| audit fields | | | |
...
#### E21 — `case_documents` (link to PS13)
...
| `document_id` | UUID FK→documents | N | PS13 object |
...
```
employees (PS01) 1──* disciplinary_cases *──1 org_units (PS01)
...
inquiry_proceedings 1──* inquiry_exhibits *──1 documents (PS13)
...
penalty_orders 1──0..1 service_register_events (PS12)
disciplinary_cases 1──* case_timeline_events (hash-chained)          (v2)
disciplinary_cases 1──* sla_pause_events                            (v2)
disciplinary_cases 1──* case_documents *──1 documents (PS13)
...
|--------|----------|---------|-----------|
| E1–E30 (case entities) | PS09 | PS09, PS14 (KPI), Auditor | PS09 roles per §3 |
| `procedure_templates`, `authority_competence` (E22/E23) | PS09 | PS09 decision guards, Auditor | SysAdmin only (reference data) |
| `employees` | PS01 | PS09 | PS01 only |
| `org_units` / `designations` / `cadres` | PS01 | PS09 | PS01 only |
| `service_register_events` | PS12 | PS09 | PS09 (append SR event, idempotent), PS12 |
| `documents` | PS13 | PS09 | PS09 (via PS13 API), PS13 |
| `notifications` | platform | PS09 | PS09 (informational only — never legal service) |
| `audit_log` | platform | Auditor, PS09 | PS09 (append on every action). Statutory tamper-evidence = **P05 dual-log (DB-trigger) + OPEN-PLAT-03 hash-chaining**, not a PS09-owned chain; E19 `case_timeline_events` is a per-case SLA/stage ledger that rides on the P05 substrate |
| `workflow_*` | platform | PS09 | PS09 |
| Payroll/pension/seniority effects | PS10/PS11/PS06 | PS09 (status echo) | PS09 emits event; owning module executes |
| DSC/eSign trust service | platform | PS09 | PS09 calls; stores signature metadata |
...
| Warning / Fine | MINOR | SR entry; fine → PS10 recovery (capped, DI-22) |
| Withholding of increment(s) | MINOR (MAJOR if cumulative effect) | PS10 pay, PS06 progression |
| Withholding of promotion | MAJOR | PS06 |
| Recovery from pay of loss caused | MINOR/MAJOR | PS10 recovery (≤1/3 pay; instalments; DCRG-only beyond retirement, DI-22) |
| Reduction to lower stage/rank | MAJOR | PS06 seniority, PS10 pay |
| Compulsory retirement | MAJOR (Art. 311(1) competence) | PS11 pension (reduced) |
| Removal from service | MAJOR (Art. 311(1) competence) | PS11 |
| Dismissal from service | MAJOR (Art. 311(1) competence) | PS11 (disqualifies future enterprise employment) |
...
7. **DI-7 Evidence seal:** every `inquiry_exhibits`/`case_documents` row stores `content_hash`; mismatch on read raises `ERR-PS09-EVIDENCE-TAMPERED`.
...
13. **DI-13 Penalty competence (v2, AI-1, R1):** finalising any `penalty_items` row requires `passed_by` to be competent for that penalty class/type per `authority_competence` for the respondent's cadre. For DISMISSAL/REMOVAL/CR, the authority must **not be subordinate to the appointing authority** (Art. 311(1)). Otherwise return `ERR-PS09-AUTHORITY-NOT-COMPETENT` (409) and refuse persistence. `competence_verified` must be true to finalise.
14. **DI-14 Mandatory consultation (v2, AI-2, R2):** finalise is blocked until every `case_consultations` row that the case's `procedure_template` marks mandatory is `CLOSED` (or `WAIVED` with reasons). Otherwise `ERR-PS09-CONSULTATION-PENDING` (409).
...
16. **DI-16 Subsistence correctness (v2, AI-6, R6):** a subsistence payment event to PS10 requires `non_employment_certificate_received=true`. If a charge-memo is not validly served by `charge_memo_due_date` (90-day window), set `deemed_review_flag=true` and escalate for review/possible revocation.
...
21. **DI-21 Hash-chained audit (v2, AI-15, R15):** each `case_timeline_events` row stores `prev_hash` (prior row's `row_hash`) and a computed `row_hash`; the verify endpoint recomputes the chain and raises `ERR-PS09-AUDIT-CHAIN-BROKEN` on any mismatch.
22. **DI-22 Recovery caps (v2, AI-20, R20):** for RECOVERY/FINE items, monthly deduction ≤ `recovery_monthly_cap_pct` (default 1/3 of pay), instalments bounded, and no recovery beyond retirement except from DCRG (`recovery_beyond_retirement` ⇒ DCRG source); violation `ERR-PS09-RECOVERY-CAP-EXCEEDED` (422).
23. **DI-23 Dispensation guard (v2, AI-3, R3):** a penalty without an inquiry is lawful only if an `inquiry_dispensations` row is `APPROVED` with a valid `reason_code` permitted by the template and recorded speaking reasons; else `ERR-PS09-DUE-PROCESS-INCOMPLETE`.
24. **DI-24 Inspection afforded (v2, AI-4, R4):** an inquiry cannot record substantive evidence (witness examination/exhibit admission) unless `list_supplied_date` and `inspection_afforded_date` are set; else `ERR-PS09-INSPECTION-NOT-AFFORDED` (409).
...
27. **DI-27 Retiree proceedings (v2, AI-13, R13):** proceedings against a retired employee require `is_retiree_case=true`, a recorded `retiree_sanction_ref`, and must respect the Rule 9 **four-year** event bar; failing the bar/sanction returns `ERR-PS09-RETIREE-PROCEEDING-BARRED` (409).
28. **DI-28 Digital signing (v2, AI-17, R17):** charge-sheets, inquiry reports, show-cause notices, and orders cannot reach a served/finalised state without `signature_type` ∈ {DSC, ESIGN}, a bound `signatory_id`, and `signed_at`; else `ERR-PS09-SIGNATURE-REQUIRED` (422).
...
### FR-PS09-001 — Complaint / Source-of-Misconduct Registration & Triage
...
- BR-4 **(v2):** Template resolution is mandatory; if no template matches jurisdiction+misconduct, `ERR-PS09-PROCEDURE-TEMPLATE-INVALID` (422).
...
**Edge Cases:** Subject is a non-employee/contractor (external subject ref); subject already retired (route to retiree-proceeding path FR-026, PS11 advisory); same allegation re-submitted (merge); **(v2)** DA resolves to the complainant ⇒ block with `ERR-PS09-DA-BIAS-CONFLICT`; HARASSMENT routes to ICC template.
...
| Failure Handling | Dup ⇒ 409 `ERR-PS09-DUPLICATE-COMPLAINT`; missing DA ⇒ 422 `ERR-PS09-DA-NOT-RESOLVED`; no template ⇒ 422 `ERR-PS09-PROCEDURE-TEMPLATE-INVALID`; DA is complainant ⇒ 409 `ERR-PS09-DA-BIAS-CONFLICT` |
| Dependencies | PS01 (employees), `procedure_templates`, `authority_competence`, notifications, audit |
...
- BR-4 **(v2):** PI material relied upon but not disclosed blocks finding (`ERR-PS09-NATURAL-JUSTICE-VIOLATION`).
...
| Failure Handling | Invalid transition ⇒ 409 `ERR-PS09-INVALID-STATE-TRANSITION`; missing doc ⇒ 422; relied-not-disclosed ⇒ 409 `ERR-PS09-NATURAL-JUSTICE-VIOLATION` |
| Dependencies | PS13 (sealed doc), notifications, SLA engine (FR-016) |
| Test Guidance | Confidentiality (charged officer cannot fetch PI report); relied-upon disclosure gate; SLA breach |

---

### FR-PS09-003 — Suspension (Parallel Status), Subsistence Allowance & Review
...
**Description:** **(v2)** Suspension is modelled as a **parallel interim status** on the case (`is_under_suspension`/`suspension_status`), **not a linear stage** — the charge/inquiry track proceeds concurrently. Issues suspension orders (ordered/deemed/continued), sets subsistence within statutory bounds, **gates the subsistence payment event on a non-employment certificate (DI-16)**, schedules mandatory periodic review, **auto-flags a deemed review when the charge-memo is not served within the 90-day window (Ajay Kumar Choudhary, DI-16)**, emits a payroll event to PS10, and supports revocation.

**Acceptance Criteria:**
1. A suspension order sets `effective_from`, `subsistence_rate_pct` ∈ [template floor, ceiling], sets case `is_under_suspension=true` (parallel) **(v2)**, and updates `employees.employment_status=SUSPENDED` via PS01 event.
2. Deemed suspension auto-creates a record when detention > 48h is recorded.
3. **(v2)** The subsistence payment event to PS10 is emitted **only** when `non_employment_certificate_received=true`; otherwise the payment is held and flagged.
...
6. Revocation sets `status=REVOKED`, `effective_to`, and emits PS10 + PS01 events; an SR event is appended for suspension and revocation (FR-013).
...
| Backend Flow | Validate bounds → persist suspension in txn → set parallel case flag → emit PS01 status event + PS12 SR event → schedule review tasks → emit PS10 subsistence event **only if NEC received** → scheduled job sets `deemed_review_flag` on 90-day lapse |
| Data Operations | INSERT/UPDATE `suspensions`; UPDATE `disciplinary_cases.is_under_suspension/suspension_status`; append via the PS12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); INSERT `workflow_actions` |
...
| Failure Handling | Out of bounds ⇒ 422 `ERR-PS09-SUBSISTENCE-OUT-OF-BOUNDS`; payment without NEC ⇒ 422 `ERR-PS09-NON-EMPLOYMENT-CERT-REQUIRED`; PS10/PS01 down ⇒ outbox + X.3 retry (backoff ×3); on exhaustion ⇒ 500 `ERR-LOADFAIL` |
| Dependencies | PS01, PS10, PS12, workflow engine, idempotency store |
| Test Guidance | Bound checks; NEC gate; deemed-review on lapse; parallel-flag (not stage); outbox retry; SR idempotency |

---

### FR-PS09-004 — Charge-Sheet / Articles of Charge (Statement of Imputations)
...
3. **(v2)** On issue, a **DSC/eSign-signed** PDF is produced (`signature_type`, `signatory_id`, `signed_at`); without a signature the sheet cannot be served (`ERR-PS09-SIGNATURE-REQUIRED`).
4. **(v2)** Service is recorded as a `legal_service_records` row; `defence_due_date` computes **only** from a `is_statutorily_valid=true` service (EMAIL non-statutory by default); invalid mode ⇒ `ERR-PS09-INVALID-SERVICE-MODE`.
...
| Backend Flow | Validate articles → persist draft → issue: render PDF via PS13, call DSC/eSign trust service, lock articles → serve: create `legal_service_records`, compute validity + due-date, mark served, notify |
...
| Failure Handling | Unsigned serve ⇒ 422 `ERR-PS09-SIGNATURE-REQUIRED`; invalid mode ⇒ 409 `ERR-PS09-INVALID-SERVICE-MODE`; PDF render fail ⇒ retain DRAFT + 500 `ERR-LOADFAIL` (X.3 retry) |
| Dependencies | PS13, DSC/eSign trust service, notifications, `procedure_templates` |
| Test Guidance | Article immutability; signature gate; valid-service computation; EMAIL non-statutory block |

---

### FR-PS09-005 — Employee's Written Statement of Defence (+ Rights & Deadlines Surface)
...
| Failure Handling | Submit after due-date ⇒ 409 `ERR-PS09-DEFENCE-WINDOW-CLOSED` (unless extension); not validly served ⇒ 403 |
...
### FR-PS09-006 — Appointment of Inquiry Officer & Presenting Officer (or ICC Constitution)
...
**Acceptance Criteria:**
1. IO, PO, DA, complainant, witnesses, and the charged officer must be mutually distinct (DI-2); violations blocked with `ERR-PS09-ACTOR-CONFLICT`.
...
5. **(v2)** For `is_posh_case`, the system enforces ICC composition (≥1 `ICC_EXTERNAL_MEMBER`, a presiding officer); missing composition ⇒ `ERR-PS09-ICC-PROCEDURE-REQUIRED`.
...
| Failure Handling | Conflict ⇒ 409 `ERR-PS09-ACTOR-CONFLICT`; POSH missing composition ⇒ 409 `ERR-PS09-ICC-PROCEDURE-REQUIRED` |
| Dependencies | PS01, notifications, `procedure_templates` |
| Test Guidance | Distinct-actor incl. DA; ICC composition; objection→replacement audit chain |

---

### FR-PS09-007 — Conduct of Departmental Inquiry (Hearings, Witnesses, Exhibits, Inspection, Ex-Parte, Stay)
...
**Acceptance Criteria:**
1. **(v2)** The IO records `list_supplied_date` and `inspection_afforded_date`; witness examination/exhibit admission is blocked until both are set (`ERR-PS09-INSPECTION-NOT-AFFORDED`).
...
**Edge Cases:** Witness hostile; exhibit authenticity challenged (objection recorded); charged officer attends late after ex-parte (IO may recall); hash mismatch ⇒ `ERR-PS09-EVIDENCE-TAMPERED`; **(v2)** evidence attempted before inspection ⇒ blocked; stay with no criminal ref ⇒ rejected.
...
| Failure Handling | Inspection missing ⇒ 409 `ERR-PS09-INSPECTION-NOT-AFFORDED`; hash mismatch ⇒ 409 `ERR-PS09-EVIDENCE-TAMPERED`; ex-parte without threshold ⇒ 422 |
| Dependencies | PS13 (sealed vault), SLA engine, notifications |
...
| Failure Handling | Missing findings ⇒ 422 `ERR-PS09-INCOMPLETE-FINDINGS`; unsigned ⇒ 422 `ERR-PS09-SIGNATURE-REQUIRED`; wrong state ⇒ 409 |
| Dependencies | PS13, DSC/eSign, notifications |
| Test Guidance | All-articles/all-respondents guard; signature; finding write-back; remit |

---

### FR-PS09-009 — Disagreement Memo & Disciplinary Authority Consideration
...
| Failure Handling | Penalty on not-proved without memo ⇒ 409 `ERR-PS09-NATURAL-JUSTICE-VIOLATION` |
| Dependencies | notifications, PS13 |
| Test Guidance | Upgrade-without-memo block; representation window |

---

### FR-PS09-010 — Show-Cause Notice on Proposed Penalty
...
| Failure Handling | Major order without closed show-cause ⇒ 409; enhancement ⇒ 409 `ERR-PS09-PENALTY-EXCEEDS-PROPOSED`; hearing denied without reason ⇒ 422 `ERR-PS09-PERSONAL-HEARING-DENIED` |
| Dependencies | notifications, personal hearings |
| Test Guidance | Subset enforcement; lapse handling; personal-hearing record |

---

### FR-PS09-011 — Penalty Order & Exoneration (Competence, Consultation, Proportionality, Recovery Caps, Signing)
...
4. **(v2)** The order is signed via DSC/eSign (`signature_type`, `signatory_id`, `signed_at`, `signature_ref`) before finalise; unsigned ⇒ `ERR-PS09-SIGNATURE-REQUIRED`.
...
**Edge Cases:** Multiple penalties of different classes (competence resolved to the **highest** class); recovery exceeding cap (validation); exoneration after long suspension (back-pay settlement to PS10); **(v2)** employee deceased before order ⇒ abate (FR-028); incompetent DA ⇒ block.
...
| Data Operations | INSERT `penalty_orders`,`penalty_items`; UPDATE case status; append via the PS12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); emit PS06/PS10/PS11 events |
...
| Failure Handling | Incomplete chain ⇒ 409 `ERR-PS09-DUE-PROCESS-INCOMPLETE`; incompetent ⇒ 409 `ERR-PS09-AUTHORITY-NOT-COMPETENT`; consultation pending ⇒ 409 `ERR-PS09-CONSULTATION-PENDING`; no proportionality ⇒ 422 `ERR-PS09-PROPORTIONALITY-REASONING-REQUIRED`; recovery cap ⇒ 422 `ERR-PS09-RECOVERY-CAP-EXCEEDED`; unsigned ⇒ 422 `ERR-PS09-SIGNATURE-REQUIRED`; edit after finalise ⇒ 409 `ERR-PS09-ORDER-IMMUTABLE` |
| Dependencies | PS06, PS10, PS11, PS12, PS13, DSC/eSign, MFA, `authority_competence`, `case_consultations` |
...
5. Decision propagates: SR supersession (FR-013) and downstream reversal/adjustment (PS06/PS10/PS11).
...
**Edge Cases:** Time-barred without condonation (reject); enhancement without show-cause (block); multiple remedies in sequence; appeal pending while recovery deducted (hold/refund with PS10).
...
| Data Operations | INSERT/UPDATE `appeals`; conditional INSERT revised `penalty_orders`; append a superseding event via the PS12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); INSERT `sla_pause_events`/`personal_hearings` |
...
| Failure Handling | Time-barred admit ⇒ 409 `ERR-PS09-APPEAL-TIME-BARRED`; incompetent authority ⇒ 409 `ERR-PS09-AUTHORITY-NOT-COMPETENT`; enhance without show-cause ⇒ 409 |
| Dependencies | PS06, PS10, PS11, PS12, MFA, personal hearings, SLA pause |
| Test Guidance | Limitation/condonation pause; set-aside reversal; enhancement guard; competence |

---

### FR-PS09-013 — Service Register Posting & Downstream Effects (Idempotent)

- **Module:** PS09-DCP
- **Primary Role(s):** Disciplinary Authority, DCP Admin, SR Custodian (PS12)
...
**Description:** Appends an immutable SR event and emits idempotent effect events to PS06/PS10/PS11. **(v3 re-grounding)** The SR ledger is the **net-new PS12 SR engine running on the P05 audit/immutability substrate** (DB-trigger, immutable, ≥ 7-yr, OPEN-PLAT-03 hash-chain) — PS09 **appends** to it via the PS12 contract, it does not own the ledger. Downstream effects flow **by reference** to PS06 (seniority/promotion), PS10 (recovery/subsistence) and PS11 (pension); the owning module executes — PS09 emits events only. **(v2)** Uses the defined **`idempotency_keys` lifecycle (DI-19, AI-11)** plus the platform **`Idempotency-Key`** header — keys minted per scope, deduplicated with a TTL, guaranteeing exactly-once effects even on double-finalise / double post-to-SR; reinforced by idempotent P01 `workflow_actions`. Outbox/retry on X.3 for reliability; supersession on appeal.

**Acceptance Criteria:**
1. On order finalise (and appeal decision), an SR event is appended with order reference (DI-10).
2. Penalty-type-specific downstream events are emitted (WITHHOLD_INCREMENT/REDUCTION → PS06+PS10; RECOVERY → PS10; CR/REMOVAL/DISMISSAL → PS11; WITHHOLD_PROMOTION → PS06).
3. **(v2)** Emission is idempotent via `idempotency_keys`; a replayed key returns the stored result and produces no second effect; mismatched fingerprint on a seen key ⇒ `ERR-PS09-IDEMPOTENCY-CONFLICT`.
...
| Entity | Use |
|--------|-----|
| `service_register_events` | append (PS12) |
...
**Edge Cases:** PS12 unavailable (outbox retains, retries); duplicate trigger (key dedup); partial downstream success (per-effect status); reduction target designation invalid (validation before emit).
...
| Backend Flow | On finalise → check/mint idempotency key (TTL) → if seen+match return snapshot; else write outbox entries (SR + per-effect) → dispatcher posts to PS12/PS06/PS10/PS11 → record correlation ids → persist response snapshot |
| Data Operations | append via the PS12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); INSERT/UPDATE `idempotency_keys`; INSERT outbox rows; UPDATE correlations |
...
| Failure Handling | Upstream down ⇒ outbox + backoff + alert; duplicate key match ⇒ replay snapshot; key mismatch ⇒ 409 `ERR-PS09-IDEMPOTENCY-CONFLICT` |
| Dependencies | PS12, PS06, PS10, PS11, idempotency store |
| Test Guidance | Double-finalise ⇒ one SR event; double post-to-SR ⇒ one effect; outbox retry; supersession; per-effect status |

---

### FR-PS09-014 — Document Evidence Vault (Link to PS13)
...
**Description:** Centralises all case artefacts as references to encrypted PS13 objects, each with SHA-256 `content_hash`, served/sealed flags, **(v2)** signature metadata, and full read-audit. Served items are visible to the charged officer; sealed items restricted; relied-upon evidence must be disclosed (DI-9).
...
3. On retrieval, the stored `content_hash` is re-verified; mismatch raises `ERR-PS09-EVIDENCE-TAMPERED` and alerts.
...
**Business Rules:**
- BR-1: Vault items never hard-deleted; supersession/versioning via PS13.
...
**Edge Cases:** Large files (chunked upload via PS13); hash mismatch (block + alert); cross-case shared exhibit (separate links).
...
| Backend Flow | Upload via PS13 → compute hash → persist link (+ signature meta) → on download verify hash + RBAC + write view-audit |
...
| Failure Handling | PS13 down ⇒ 500 `ERR-LOADFAIL` (X.3 retry); hash mismatch ⇒ 409 `ERR-PS09-EVIDENCE-TAMPERED` |
| Dependencies | PS13, audit |
| Test Guidance | Hash verify; served+disclosed visibility; read-audit on sealed |

---

### FR-PS09-015 — Integrity Register & Vigilance Clearance (+ Sealed Cover)
...
**Description:** Maintains a per-employee vigilance record (clearance status, integrity grade, **(v2)** sealed-cover flag) driven by case lifecycle, and exposes a clearance API consumed by PS06/PS11. **(v2)** When proceedings are pending, PS06 promotion recommendations are held in **sealed cover** (AI-13).
...
5. **(v2)** `sealed_cover_flag` is set when proceedings pending and surfaced to PS06.
...
- BR-2: `NOT_CLEAR`/`UNDER_PROCEEDINGS`/sealed-cover blocks promotion in PS06.
...
**Edge Cases:** Multiple concurrent cases (most-restrictive wins); clearance during pending appeal (UNDER_PROCEEDINGS retained); retiring employee clearance for pension (PS11 consumes).
...
| Authorization | Read: RBAC (PS06/PS11 service + Vigilance/DA); write: Vigilance Officer |
| State Changes & Side Effects | Status + sealed-cover drive PS06 promotion gate, PS11 pension clearance |
| Failure Handling | Concurrent updates ⇒ most-restrictive resolver; 409 on stale write |
| Dependencies | PS06, PS11, audit |
| Test Guidance | Event-driven derivation; most-restrictive; sealed-cover; override audit |

---

### FR-PS09-016 — SLA / Statutory-Timeline Tracking, Proportionality Analytics & Case Analytics
...
| Authorization | Detail: DA/Admin/Vigilance; aggregates: Auditor; PS14 service-account |
| State Changes & Side Effects | sla_status updates; escalation notifications; PS14 feed |
| Failure Handling | Evaluator failure ⇒ retry + alert; stale read-model ⇒ refresh |
| Dependencies | scheduler, notifications, PS14, `sla_pause_events`, `procedure_templates` |
| Test Guidance | Target computation; at-risk/breach with pause skip; ageing; proportionality outlier correctness |

---

### FR-PS09-017 — Procedure Template & Jurisdiction Overlay (Two-Layer Model) **(v2, AI-8, R8)**
...
**Edge Cases:** No matching template at case creation (`ERR-PS09-PROCEDURE-TEMPLATE-INVALID`); template edited mid-case (new cases only; open cases keep pinned version); conflicting active selectors (validation blocks activation).
...
| Failure Handling | No match ⇒ 422 `ERR-PS09-PROCEDURE-TEMPLATE-INVALID`; kernel-disable attempt ⇒ 422 `VALIDATION_FAILED` |
...
### FR-PS09-018 — Authority Competence Matrix & Finalise-Time Competence Guard **(v2, AI-1, R1)**
...
3. **A `DISMISSAL` finalised by an authority subordinate to the appointing authority returns `ERR-PS09-AUTHORITY-NOT-COMPETENT` (409) and cannot persist** (the canonical failing-then-passing test).
...
**Edge Cases:** Cadre changes by promotion mid-case (snapshot used); no matrix entry for cadre/class (`ERR-PS09-AUTHORITY-NOT-COMPETENT`, fail-safe deny); delegated authority (modelled as authority level).
...
| Failure Handling | 409 `ERR-PS09-AUTHORITY-NOT-COMPETENT`; fail-safe deny on missing entry |
| Dependencies | PS01 (appointing authority resolution), `procedure_templates` |
| Test Guidance | **Canonical:** DISMISSAL by subordinate authority blocked; highest-class resolution; missing-entry deny |

---

### FR-PS09-019 — Statutory Consultation Management (UPSC / CVC / ICC / Legal) **(v2, AI-2, R2)**
...
3. Finalise is blocked with `ERR-PS09-CONSULTATION-PENDING` until all mandatory consultations are CLOSED/WAIVED.
...
| Failure Handling | Finalise with pending ⇒ 409 `ERR-PS09-CONSULTATION-PENDING` |
| Dependencies | `procedure_templates`, FR-024, PS13 |
| Test Guidance | Required-set derivation; finalise gate; pause; relied-advice disclosure |

---

### FR-PS09-020 — Dispense-with-Inquiry (Article 311(2) Provisos) **(v2, AI-3, R3)**
...
| Failure Handling | Invalid reason ⇒ 422 `ERR-PS09-INQUIRY-DISPENSATION-INVALID`; penalty without approved dispensation/inquiry ⇒ 409 `ERR-PS09-DUE-PROCESS-INCOMPLETE` |
...
### FR-PS09-021 — Legal Service of Documents & Digital Signing (DSC/eSign) **(v2, AI-5, AI-17, R5, R17)**
...
- BR-3: Service proof is stored as an PS13 object with hash.
...
| Failure Handling | Invalid mode ⇒ 409 `ERR-PS09-INVALID-SERVICE-MODE`; unsigned ⇒ 422 `ERR-PS09-SIGNATURE-REQUIRED`; verify fail ⇒ 409 `ERR-PS09-SIGNATURE-INVALID` |
| Dependencies | DSC/eSign trust service, PS13, `procedure_templates` |
| Test Guidance | Valid-service window computation; EMAIL non-statutory; signing gate; verify |

---

### FR-PS09-022 — Common / Joint (Multi-Respondent) Proceedings **(v2, AI-7, R7)**
...
| Failure Handling | Close with unresolved respondent ⇒ 409 `ERR-PS09-RESPONDENT-UNRESOLVED`; conflicting actor ⇒ 409 `ERR-PS09-ACTOR-CONFLICT` |
...
### FR-PS09-023 — POSH / Internal Committee Procedure **(v2, AI-16, R16)**
...
2. ICC constitution enforces composition (presiding officer + ≥1 external member); missing ⇒ `ERR-PS09-ICC-PROCEDURE-REQUIRED`.
...
| Failure Handling | Missing composition ⇒ 409 `ERR-PS09-ICC-PROCEDURE-REQUIRED` |
...
### FR-PS09-024 — SLA Pause/Resume & Clock Management **(v2, AI-10, AI-18, R10, R18)**
...
| Failure Handling | Resume without pause ⇒ 409 `ERR-PS09-SLA-PAUSE-INVALID` |
| Dependencies | FR-016, FR-007/008/012/019 |
| Test Guidance | No false breach while paused; recompute on resume; coalesced overlaps |

---

### FR-PS09-025 — Personal Hearing Record (Show-Cause / Appeal) **(v2, AI-21)**
...
| Failure Handling | Deny without reason ⇒ 422 `ERR-PS09-PERSONAL-HEARING-DENIED` |
| Dependencies | FR-010, FR-012 |
| Test Guidance | Grant/deny reasons; minutes; linkage |

---

### FR-PS09-026 — Mid-Proceeding Jurisdiction Transfer, Sealed Cover & Retiree Proceedings **(v2, AI-13, R13)**
...
**Description:** Handles employee-lifecycle races against long proceedings: a **jurisdiction-transfer** event re-resolves the competent DA (from PS05/PS06 changes) and records `JURISDICTION_TRANSFER` in the timeline; **sealed cover** freezes PS06 promotion recommendations while proceedings are pending; **retiree proceedings** enforce the CCS (Pension) Rule 9 **four-year event bar** plus required enterprise/President sanction (DI-27).
...
2. While proceedings are pending, PS06 promotion is held in **sealed cover** (`vigilance_records.sealed_cover_flag`, `disciplinary_cases.is_sealed_cover`); exoneration lifts it, penalty resolves it per rules.
3. Retiree proceedings require `is_retiree_case=true`, a recorded `retiree_sanction_ref`, and that the misconduct event be within the Rule 9 four-year bar; failing ⇒ `ERR-PS09-RETIREE-PROCEEDING-BARRED`.
...
| `vigilance_records` | sealed-cover flag (PS06) |
...
**UI Behavior Notes:** Jurisdiction-transfer banner with old/new DA; sealed-cover badge feeding PS06; retiree-proceeding panel with sanction ref and four-year-bar check.
...
| Backend Flow | On PS05/PS06 event → re-resolve DA → record event; set sealed cover while pending; on retiree mark, enforce Rule 9 + sanction |
...
| Failure Handling | Barred retiree ⇒ 409 `ERR-PS09-RETIREE-PROCEEDING-BARRED`; missing sanction ⇒ 422 |
| Dependencies | PS05, PS06, PS11, FR-015, FR-018 |
| Test Guidance | DA re-resolution; sealed-cover gate; Rule 9 bar + sanction |

---

### FR-PS09-027 — Court-Grade Hash-Chained Audit & Verification **(v2, AI-15, R15)**
...
**Description (v3 re-grounded):** Tamper-evidence is **not** an invented parallel audit engine. The authoritative immutable record is the **platform P05 dual log** (`audit_log` + `security_audit_log`) captured by **DB trigger** on every mutation (immutable, PII-masked, ≥ 7-yr, 100% capture); **statutory-grade hash-chaining is the platform's OPEN-PLAT-03 mechanism** (periodic hash-chaining of audit partitions, chain head exported to WORM) which PS09 **tracks and consumes rather than re-implements**. The PS09 `case_timeline_events` ledger is a **domain SLA/stage ledger** layered on P05: each row stores `seq_no`, `prev_hash` (prior row's `row_hash`) and a computed `row_hash` (DI-21) to give per-case verifiability for court production, and the **verify endpoint** recomputes the chain and cross-checks against P05/OPEN-PLAT-03. Where v2 implied PS09 owned the cryptographic chain, v3 re-anchors it onto the platform substrate.
...
4. Any DB-layer mutation of a row is detected on verify (`ERR-PS09-AUDIT-CHAIN-BROKEN`).
...
| Failure Handling | Mismatch ⇒ 409 `ERR-PS09-AUDIT-CHAIN-BROKEN` with first broken seq |
| Dependencies | platform audit |
| Test Guidance | Chain continuity; tamper detection; concurrent insert ordering |

---

### FR-PS09-028 — Abatement on Death / Terminal Handling **(v2, AI-14, R14)**
...
- BR-3: Abatement does not by itself decide pension — PS11 applies its own rules.
...
| Data Operations | UPDATE `disciplinary_cases`/`case_respondents`; INSERT `penalty_orders`(ABATED); append the abatement event via the PS12 ingestion contract `POST /api/v1/sr/ingest` (FR-02) |
...
| Failure Handling | Penalty finalise on abated ⇒ 409 `ERR-PS09-CASE-ABATED` |
| Dependencies | PS01, PS11, FR-013, FR-016 |
...
- Base path `/api/v1/dcp`; JSON; OIDC JWT bearer carrying resolved roles + tenant/entity scope; RBAC + row-level + field-level enforcement via **`Authorization.check` (P02)** — endpoints never re-implement permission logic.
- **Cursor pagination only** (platform convention, Foundation §1): `?limit=` (default 25, max 100) + `cursor=`; response carries `next_cursor`. Offset/`page` paging is **not** used. `?sort=field:asc|desc`.
- Every request carries / is assigned **`X-Correlation-Id`**, echoed in the response header and written to every P05 audit/log line.
- **Idempotency lifecycle (DI-19, AI-11):** state-changing propagating posts (`/suspensions`, `/orders/{id}/finalise`, `/orders/{id}/post-to-sr`, `/appeals/{id}/decide`) **require** an `Idempotency-Key` header (platform 24h replay; domain `idempotency_keys` ledger for exactly-once propagation). The server records the key in `idempotency_keys` with a `request_fingerprint` (SHA-256 of canonical body), `scope`, `status`, and `expires_at` (**TTL = 7 days**). Re-sending a known key + matching fingerprint **replays the stored response** with no second effect; a known key with a **different** fingerprint returns `ERR-PS09-IDEMPOTENCY-CONFLICT` (409). Keys may be client-minted (UUID) or server-minted if absent; expired keys are pruned.
...
Platform envelope (Foundation §1): the body carries `{ error: { code, message, field, details } }`; the correlation id is the **`X-Correlation-Id` response header**, not a body `requestId`.

```json
// 409 Conflict  (header: X-Correlation-Id: req-7f3a9c20)
{
  "error": { "code": "ERR-PS09-AUTHORITY-NOT-COMPETENT", "message": "Signing authority is subordinate to the appointing authority for a DISMISSAL penalty (Article 311(1)).", "field": "passed_by", "details": { "penaltyType": "DISMISSAL", "requiredLevel": "APPOINTING_AUTHORITY" } }
}
```

**Shared codes (platform 8-code table, Foundation §1):** `VALIDATION_FAILED (422)`, `UNAUTHENTICATED (401)`, `FORBIDDEN (403)`, `NOT_FOUND (404)`, `CONFLICT (409)`, `PRECONDITION_FAILED (412)`, `RATE_LIMITED (429)`, `INTERNAL (500)`. The module-specific codes below are domain refinements returned alongside these; legacy `VALIDATION_ERROR`/`AUTH_REQUIRED`/`INTERNAL_ERROR`/`UPSTREAM_UNAVAILABLE` are retired.

### 10.3 Error-code catalog (module-specific, in addition to shared codes)

Module error codes adopt the **`ERR-PS09-*` namespace** (Foundation §8.3 `ERR-<enterprise>-*` convention; registered in the Foundation §5 catalogue). Each `ERR-PS09-*` id **maps onto exactly one of the platform 8 standard HTTP codes** (`VALIDATION_FAILED 422`, `UNAUTHENTICATED 401`, `FORBIDDEN 403`, `NOT_FOUND 404`, `CONFLICT 409`, `PRECONDITION_FAILED 412`, `RATE_LIMITED 429`, `INTERNAL 500`) and carries a `MSG-PS09-*` user-message id. No bare `UPPER_SNAKE` module codes, and no invented `VALIDATION_ERROR`/`UPSTREAM_UNAVAILABLE`/`503`, appear in live sections. `ERR-PS09-SIGNATURE-INVALID` is namespaced to resolve the legacy cross-module bare `SIGNATURE_INVALID` collision (R4 Finding 3); `ERR-PS09-IDEMPOTENCY-CONFLICT` specialises the platform `CONFLICT 409` / `ERR-DUP-INSTANCE` semantics (R4 Finding 6).

| `ERR-PS09-*` code | HTTP (standard) | User message id | Meaning | New v2 |
| --- | --- | --- | --- | :--: |
| `ERR-PS09-DUPLICATE-COMPLAINT` | 409 CONFLICT | `MSG-PS09-DUPLICATE-COMPLAINT` | Open complaint/case already exists for subject | |
| `ERR-PS09-DA-NOT-RESOLVED` | 422 VALIDATION_FAILED | `MSG-PS09-DA-NOT-RESOLVED` | Competent disciplinary authority not resolvable | |
| `ERR-PS09-DA-BIAS-CONFLICT` | 409 CONFLICT | `MSG-PS09-DA-BIAS-CONFLICT` | DA is also complainant/witness (DI-2) | ✔ |
| `ERR-PS09-INVALID-STATE-TRANSITION` | 409 CONFLICT | `MSG-PS09-INVALID-STATE-TRANSITION` | Stage/status transition not allowed | |
| `ERR-PS09-ACTOR-CONFLICT` | 409 CONFLICT | `MSG-PS09-ACTOR-CONFLICT` | DA/IO/PO/witness not mutually distinct | |
| `ERR-PS09-DEFENCE-WINDOW-CLOSED` | 409 CONFLICT | `MSG-PS09-DEFENCE-WINDOW-CLOSED` | Defence submitted after due date without extension | |
| `ERR-PS09-INSPECTION-NOT-AFFORDED` | 409 CONFLICT | `MSG-PS09-INSPECTION-NOT-AFFORDED` | Evidence recorded before document/witness inspection (DI-24) | ✔ |
| `ERR-PS09-INCOMPLETE-FINDINGS` | 422 VALIDATION_FAILED | `MSG-PS09-INCOMPLETE-FINDINGS` | Inquiry report missing an article/respondent finding | |
| `ERR-PS09-NATURAL-JUSTICE-VIOLATION` | 409 CONFLICT | `MSG-PS09-NATURAL-JUSTICE-VIOLATION` | Penalty on not-proved without memo; or relied-not-disclosed material (DI-9) | |
| `ERR-PS09-PENALTY-EXCEEDS-PROPOSED` | 409 CONFLICT | `MSG-PS09-PENALTY-EXCEEDS-PROPOSED` | Order penalty not subset of show-cause (DI-4) | |
| `ERR-PS09-DUE-PROCESS-INCOMPLETE` | 409 CONFLICT | `MSG-PS09-DUE-PROCESS-INCOMPLETE` | Required prior stage missing / no lawful dispensation (DI-3/DI-23) | |
| `ERR-PS09-AUTHORITY-NOT-COMPETENT` | 409 CONFLICT | `MSG-PS09-AUTHORITY-NOT-COMPETENT` | Signing authority not competent for penalty class/type (DI-13, Art. 311(1)) | ✔ |
| `ERR-PS09-CONSULTATION-PENDING` | 409 CONFLICT | `MSG-PS09-CONSULTATION-PENDING` | Mandatory UPSC/CVC/ICC consultation not closed (DI-14) | ✔ |
| `ERR-PS09-INQUIRY-DISPENSATION-INVALID` | 422 VALIDATION_FAILED | `MSG-PS09-INQUIRY-DISPENSATION-INVALID` | Dispensation reason not permitted by template (DI-23) | ✔ |
| `ERR-PS09-INVALID-SERVICE-MODE` | 409 CONFLICT | `MSG-PS09-INVALID-SERVICE-MODE` | Service mode not statutorily valid per template (DI-15) | ✔ |
| `ERR-PS09-SIGNATURE-REQUIRED` | 422 VALIDATION_FAILED | `MSG-PS09-SIGNATURE-REQUIRED` | Artefact lacks DSC/eSign (DI-28) | ✔ |
| `ERR-PS09-SIGNATURE-INVALID` | 409 CONFLICT | `MSG-PS09-SIGNATURE-INVALID` | Signature verification failed (namespaced; resolves collision, R4 Finding 3) | ✔ |
| `ERR-PS09-NON-EMPLOYMENT-CERT-REQUIRED` | 422 VALIDATION_FAILED | `MSG-PS09-NON-EMPLOYMENT-CERT-REQUIRED` | Subsistence payment without NEC (DI-16) | ✔ |
| `ERR-PS09-PROPORTIONALITY-REASONING-REQUIRED` | 422 VALIDATION_FAILED | `MSG-PS09-PROPORTIONALITY-REASONING-REQUIRED` | Order finalise without proportionality reasoning (DI-20) | ✔ |
| `ERR-PS09-RECOVERY-CAP-EXCEEDED` | 422 VALIDATION_FAILED | `MSG-PS09-RECOVERY-CAP-EXCEEDED` | Recovery beyond cap/instalment/retirement rule (DI-22) | ✔ |
| `ERR-PS09-ICC-PROCEDURE-REQUIRED` | 409 CONFLICT | `MSG-PS09-ICC-PROCEDURE-REQUIRED` | POSH case missing ICC composition (AI-16) | ✔ |
| `ERR-PS09-RESPONDENT-UNRESOLVED` | 409 CONFLICT | `MSG-PS09-RESPONDENT-UNRESOLVED` | Case close with an unresolved active respondent (DI-25) | ✔ |
| `ERR-PS09-RETIREE-PROCEEDING-BARRED` | 409 CONFLICT | `MSG-PS09-RETIREE-PROCEEDING-BARRED` | Retiree proceeding beyond four-year bar or without sanction (DI-27) | ✔ |
| `ERR-PS09-CASE-ABATED` | 409 CONFLICT | `MSG-PS09-CASE-ABATED` | Penalty finalise on an abated case/respondent (DI-26) | ✔ |
| `ERR-PS09-SLA-PAUSE-INVALID` | 409 CONFLICT | `MSG-PS09-SLA-PAUSE-INVALID` | Resume without an open pause (DI-18) | ✔ |
| `ERR-PS09-IDEMPOTENCY-CONFLICT` | 409 CONFLICT | `MSG-PS09-IDEMPOTENCY-CONFLICT` | Known idempotency key with mismatched fingerprint — specialises `ERR-DUP-INSTANCE` (DI-19) | ✔ |
| `ERR-PS09-AUDIT-CHAIN-BROKEN` | 409 CONFLICT | `MSG-PS09-AUDIT-CHAIN-BROKEN` | Per-case timeline-chain verification failed against P05/OPEN-PLAT-03 (DI-21) | ✔ |
| `ERR-PS09-PROCEDURE-TEMPLATE-INVALID` | 422 VALIDATION_FAILED | `MSG-PS09-PROCEDURE-TEMPLATE-INVALID` | No/ambiguous procedure template (DI-17) | ✔ |
| `ERR-PS09-ORDER-IMMUTABLE` | 409 CONFLICT | `MSG-PS09-ORDER-IMMUTABLE` | Edit attempted on finalised order (DI-6) | |
| `ERR-PS09-APPEAL-TIME-BARRED` | 409 CONFLICT | `MSG-PS09-APPEAL-TIME-BARRED` | Appeal beyond limitation without condonation (DI-5) | |
| `ERR-PS09-EVIDENCE-TAMPERED` | 409 CONFLICT | `MSG-PS09-EVIDENCE-TAMPERED` | content_hash mismatch (DI-7) | |
| `ERR-PS09-SUBSISTENCE-OUT-OF-BOUNDS` | 422 VALIDATION_FAILED | `MSG-PS09-SUBSISTENCE-OUT-OF-BOUNDS` | Subsistence rate outside template bounds (DI-8) | |
| `ERR-PS09-PERSONAL-HEARING-DENIED` | 422 VALIDATION_FAILED | `MSG-PS09-PERSONAL-HEARING-DENIED` | Requested personal hearing denied without reasons (DI-29) | ✔ |
| `ERR-PS09-CONFIDENTIALITY-DENIED` | 403 FORBIDDEN | `MSG-PS09-CONFIDENTIALITY-DENIED` | Attempt to access sealed/confidential artefact | |
...
// 409 Conflict
{
  "error": { "code": "ERR-PS09-AUTHORITY-NOT-COMPETENT", "message": "A DISMISSAL must be passed by an authority not subordinate to the appointing authority (Article 311(1)).", "field": "passed_by" }
}
```
...
// 409 Conflict
{ "error": { "code": "ERR-PS09-INVALID-SERVICE-MODE", "message": "EMAIL is not a statutorily valid service mode for template CCS_CCA_2026.", "field": "serviceMode" } }
```
...
```json
// 409 Conflict
{ "error": { "code": "ERR-PS09-CONSULTATION-PENDING", "message": "UPSC consultation must be CLOSED before finalising a major penalty.", "field": "consultations" } }
```
...
|--------|---------|-----------------|-------------|
| PS12 SR | order finalise, suspension, appeal, **abatement** | event_type, employee_id, order_ref, effective_date | append-only, **idempotent (key)** |
| PS10 PAY | suspension (**NEC-gated**), recovery (**capped**), reduction, withhold-increment | employee_id, effect_type, amount/%/effective_date | outbox + retry, idempotent |
| PS11 PEN | CR/removal/dismissal, pension cut, **retiree** | employee_id, effect_type, value | outbox + retry |
| PS06 PPP | reduction in rank, withhold promotion, **sealed cover** | employee_id, designation/seniority effect, sealed_cover_flag | outbox + retry |
| PS05 TRJ | transfer (inbound) | employee_id, new org_unit ⇒ **jurisdiction transfer** | event consume |
| PS13 DMS | all artefacts (**signed**) | object upload, hash, signature metadata | synchronous + retry |
| DSC/eSign trust service | sign/verify | artefact hash, signatory, timestamp | synchronous |
| PS14 DAS | analytics incl. **proportionality outliers** | KPI read feed | read-only |
...
|----------|-------------|
| Performance | Platform baseline (Vision §2.9): standard API **p95 < 500 ms @ 300 concurrent**; read-heavy p95 < 300 ms cached / < 1000 ms uncached; writes p95 < 1500 ms; analytics aggregates via materialised views ≤ 15 min; competence/consultation P01 guards add ≤ 50 ms to finalise |
| Availability | **99.5%/month** uptime; **RPO < 1 h, RTO < 4 h** (platform baseline — replaces the invented 99.9% / RPO ≤ 15 min) |
| Scalability | Horizontal scaling; handle 100k+ active cases and multi-respondent proceedings; **cursor-paginated** everywhere; per-tenant isolation |
| Security | OWASP; TLS 1.2+; encryption at rest; **MFA enforced for high-privilege statutory roles** + step-up MFA for major penalties/appeals; **DSC/eSign** for statutory signing; field-level confidentiality enforced by P02 (mask on serialization) |
| Privacy | DPDP Act 2023 / DPDPA consent (`consent_records`); PII minimisation; whistle-blower protection; **POSH heightened confidentiality**; masked exports; PII stripped before any P03 chat grounding |
| Auditability | **P05 dual log** (`audit_log` + `security_audit_log`), DB-trigger capture, immutable, PII-masked, **≥ 7-yr retention, 100% mutation capture**; tamper-evidence via **OPEN-PLAT-03 hash-chaining** (chain head → WORM); domain `case_timeline_events` SLA/stage ledger + verify endpoint ride on this substrate; read-audit on sealed records |
| Integrity | SHA-256 evidence sealing; immutable finalised orders; **exactly-once** outbox propagation via idempotency keys + idempotent P01 actions; **no hard delete (soft delete only)** |
| Legality | Competence guard (Art. 311(1)), mandatory-consultation gate, lawful dispensation, valid-service vs notification separation enforced server-side as P01 stage guards |
| Accessibility | WCAG 2.1 AA across all screens incl. Rights & Deadlines surface and fast lane; responsive 375/768/1280 px, touch ≥ 44×44 px |
| Retention | Statutory retention of disciplinary records (lifelong + post-service per schedule, never below the statutory floor; right-to-erasure does not apply to statutory data); legal-hold support; signed-artefact + signature-metadata retention |
| Observability | Structured logs (no PII values, per §0.5), metrics on SLA breaches/pauses, competence/consultation blocks, propagation failures; traces correlated by **`X-Correlation-Id`**; `JOB-PS09-*` run audit rows |
...
| **Sealed cover set/lifted** | DA, Vigilance, PS06 | in-app | Promotion sealed cover {status} |
...
|--------------|-------------|-----------|
| Caseload & status mix | Open/closed by status (incl. ABATED), case_type, org_unit | DA, Admin, PS14 |
...
| Penalty mix | Distribution by penalty_type/class | DA, PS14 |
| **Penalty proportionality outliers (v2)** | Orders flagged where penalty severity is unusual vs misconduct/precedent (AI-12) | DA, Auditor, PS14 |
| Exoneration rate | Exonerated ÷ concluded | Auditor, PS14 |
| Appeal overturn rate | Set-aside+modified ÷ appeals decided | Auditor, PS14 |
| Average cycle time | Initiation → closure (pause-adjusted) | DA, PS14 |
...
| Vigilance clearance dashboard | Clearance status distribution; doubtful-integrity & **sealed-cover** lists | Vigilance, PS06/PS11 |
...
| M-3 | Migrate open cases with pinned template + current stage + key artefacts (scanned to PS13 with hashes **and signature metadata where available**); closed cases as historical |
...
| M-5 | Link suspensions/penalties to existing PS10/PS11/PS06 effects without re-triggering (migration flag suppresses re-emit; idempotency keys seeded) |
...
- **Build order honours the Council "One Thing To Do First":** seed `authority_competence` + `procedure_templates` and prove the canonical failing-then-passing test (`DISMISSAL` by subordinate authority ⇒ `ERR-PS09-AUTHORITY-NOT-COMPETENT`) before any decision logic ships.
- Dual-run period: new intake on PS09; legacy frozen read-only.
...
- Integration smoke tests with PS05/PS06/PS10/PS11/PS12/PS13 + DSC/eSign trust service green.
...
|----|----------|----------|--------------|--------------|
| FR-PS09-001 | case_complaints, disciplinary_cases, case_respondents, procedure_templates | /complaints, /triage | PS01, templates, competence, audit | §12.1 |
| FR-PS09-002 | preliminary_inquiries, inquiry_exhibits | /preliminary-inquiries | PS13, SLA | §12.1 |
| FR-PS09-003 | suspensions | /suspensions, /non-employment-certificate | PS01, PS10, PS12 | §12.6/12.7 |
| FR-PS09-004 | charge_sheets, charge_articles, legal_service_records | /charge-sheets | PS13, DSC/eSign, templates | §12.2 |
| FR-PS09-005 | defence_statements | /defence, /my-rights | scheduler | §12.1 |
| FR-PS09-006 | inquiry_appointments, inquiry_proceedings | /appointments, /icc | PS01, templates | §12.3 |
| FR-PS09-007 | inquiry_hearings, inquiry_witnesses, inquiry_exhibits, sla_pause_events | /hearings,/witnesses,/exhibits,/supply-list,/stay | PS13, SLA | §12.3 |
| FR-PS09-008 | inquiry_reports, case_respondents | /report | PS13, DSC/eSign | §12.3 |
| FR-PS09-009 | disagreement_memos, legal_service_records | /disagreement-memo | notifications | §12.1 |
| FR-PS09-010 | show_cause_notices, personal_hearings | /show-cause | notifications | §12.1 |
| FR-PS09-011 | penalty_orders, penalty_items, authority_competence, case_consultations | /orders,/sign,/finalise | PS06/PS10/PS11/PS12/PS13, MFA, DSC/eSign | §12.4 |
| FR-PS09-012 | appeals, personal_hearings, sla_pause_events | /appeals | PS06/PS10/PS11/PS12 | §12.5 |
| FR-PS09-013 | service_register_events, idempotency_keys, penalty_orders | /post-to-sr | PS12/PS06/PS10/PS11 | §12.4 |
| FR-PS09-014 | case_documents, inquiry_exhibits | /documents | PS13 | — |
| FR-PS09-015 | vigilance_records | /vigilance | PS06/PS11 | — |
| FR-PS09-016 | case_timeline_events, sla_pause_events | /timeline,/analytics | scheduler, PS14 | all |
| FR-PS09-017 | procedure_templates, authority_competence | /admin/procedure-templates | — | §12.1 |
| FR-PS09-018 | authority_competence, penalty_orders, case_respondents | /admin/authority-competence,/competence-check | PS01 | §12.4 |
| FR-PS09-019 | case_consultations, sla_pause_events | /consultations | templates, PS13 | §12.9 |
| FR-PS09-020 | inquiry_dispensations, inquiry_proceedings | /dispensation | templates | §12.1 |
| FR-PS09-021 | legal_service_records, case_documents, penalty_orders | /legal-service,/sign,/verify | DSC/eSign, PS13, templates | §12.2/12.4 |
| FR-PS09-022 | case_respondents, inquiry_reports, penalty_orders | /respondents | FR-007/008/011 | §12.1 |
| FR-PS09-023 | inquiry_appointments, inquiry_proceedings, procedure_templates | /icc,/conciliation | FR-006/007/008/011 | §12.3 |
| FR-PS09-024 | sla_pause_events, case_timeline_events | /sla/pause,/sla/resume | FR-016 | §12.8 |
| FR-PS09-025 | personal_hearings, show_cause_notices, appeals | /personal-hearings | FR-010/012 | — |
| FR-PS09-026 | disciplinary_cases, case_respondents, vigilance_records | /jurisdiction-transfer,/sealed-cover,/retiree-proceeding | PS05/PS06/PS11 | §12.1 |
| FR-PS09-027 | case_timeline_events, audit_log | /audit/verify,/audit/chain | platform audit | — |
| FR-PS09-028 | disciplinary_cases, case_respondents, penalty_orders, service_register_events | /abate | PS01/PS11/PS12 | §12.1/12.4 |
...
| Downstream integration (PS05/PS06/PS10/PS11/PS12/PS13/PS14 + DSC/eSign) | mapped | yes | 0 |
| **Platform: workflow on P01** (no parallel engine; `workflow_actions` not `workflow_actions`) | required | due-process = configured P01/W.1 flows | 0 |
| **Platform: RBAC on P02 / RBAC v1.7** (roles as ADDITIONS; SoD via P01/P02) | required | §3.1 mapping + capability flags | 0 |
| **Platform: audit on P05** (dual log, DB-trigger, immutable, 7-yr) + OPEN-PLAT-03 hash-chain | required | §4 / §11 / FR-027 re-grounded | 0 |
| **Platform: confidentiality/masking on P02** (PII ceiling, field mask on serialization) | required | §3.3 / §2.2 | 0 |
| **Platform: SR ledger = PS12 on P05 substrate** (append-only, idempotent) | required | FR-013 / §10.5 | 0 |
| **Platform: notifications on X.2** (EMAIL mandatory for statutory notices) | required | §13 / §2.2 / `MSG-PS09-*` | 0 |
| **Platform: background jobs on X.1** (`JOB-PS09-SLA/TIMELINE/EFFDATE` for SLA pause/resume) | required | §4 / §2.2; Foundation §4 index | 0 |
| **Platform: API conventions** (`/api/v1`, Idempotency-Key, cursor pagination, X-Correlation-Id) | required | §4 / §10.1 | 0 |
| **Platform: error envelope + 8-code table** (`VALIDATION_FAILED 422` … `INTERNAL 500`; `details`; header correlation id) | required | §4 / §10.2 | 0 |
| **Platform: multi-tenancy** (`tenant_id`/`entity_id` on every entity; data-layer scoping) | required | §2.2 / §4 / header | 0 |
| **Platform: NFR baseline** (99.5%/month, RPO < 1 h, p95 < 500 ms, WCAG 2.1 AA, soft-delete only) | required | §4 / §11 | 0 |
| **Platform: migration on P06** (ETL+V, 3 dry runs, `<enterprise>_source_id`) | required | §4 / §15 | 0 |
| **Platform: PS-code scheme (PS09) + cross-module references re-keyed** | required | applied throughout | 0 |
| **Alignment section (FR → platform service map)** | required | `## Alignment with PrimeSoft Platform` | 0 |
| **Amendments (v2 → v3) table** | required | `## Amendments (v2 → v3: platform re-grounding)` | 0 |
| Shared-foundation reuse → **platform-foundation consumption** (no redefinition) | required | honoured (PLATFORM_FOUNDATION / MODULE_RECONCILIATION) | 0 |
...
**Result: 0 unresolved gaps (functional + platform re-grounding rows).**

---

## Alignment with PrimeSoft Platform

This module is **NET-NEW (enterprise-specific)** statutory due-process (`MODULE_RECONCILIATION.md` §A row PS09, §D): the case/charge/inquiry/penalty/appeal **business logic** is authored here, but it **runs on platform engines** and consumes platform contracts by id — it is not a parallel platform. Per `PLATFORM_FOUNDATION.md` §9, enterprise BRDs author only the net-new statutory engine and run it on P01/P05/P06.

### FR → platform-service map

| FR | What it authors (enterprise-specific) | Runs on / consumes (platform) |
|----|--------------------------------|-------------------------------|
| FR-PS09-001 Complaint & triage | complaint intake, template/DA resolution | **P02** (auth/scope), **P05** (audit), `VAL-PS09-*`, X.2 notify |
| FR-PS09-002 Preliminary inquiry | PI workflow + relied-material disclosure | **P01** (PI flow + SLA), **P05**, **PS13** (sealed report) |
| FR-PS09-003 Suspension & subsistence | suspension status, NEC gate, deemed review | **P01** (order flow), **PS10** subsistence event (NEC-gated), **PS12** SR event, **X.1** review jobs |
| FR-PS09-004 Charge-sheet | articles, track, valid service | **P01**, **PS13** (signed PDF), DSC/eSign via **X.3**, `legal_service_records` |
| FR-PS09-005 Defence & rights surface | statement of defence, rights/deadlines | **P02**, X.2, **X.1** deadline jobs |
| FR-PS09-006 IO/PO / ICC appointment | appointments with SoD | **P01** (DYNAMIC_APPROVER), **P02** (distinct-actor enforcement) |
| FR-PS09-007 Inquiry conduct | hearings, witnesses, exhibits, stay, inspection | **P01** (inquiry stage + STAYED + SLA pause via `JOB-PS09-SLA/TIMELINE`), **PS13** vault, **P05** |
| FR-PS09-008 Inquiry report | per-respondent findings, signing | **P01**, **PS13**, DSC/eSign (**X.3**) |
| FR-PS09-009 Disagreement memo | DA disagreement + representation | **P01**, X.2, `legal_service_records` |
| FR-PS09-010 Show-cause + personal hearing | proposed-penalty notice, hearing record | **P01**, X.2 |
| FR-PS09-011 Penalty order & exoneration | competence/consultation/proportionality/recovery-cap guards, signing | **P01** (stage-entry guards + finalise + MFA), **P05**, **PS12** SR append, effects to **PS06/PS10/PS11**, DSC/eSign (**X.3**) |
| FR-PS09-012 Appeal / revision / review | remedy flow, limitation, competence | **P01** (appeal flow; authority ≠ DA), **P02**, **PS12** supersession |
| FR-PS09-013 SR posting & downstream effects | exactly-once propagation | **PS12 SR ledger on P05 substrate**; effects by reference to **PS06/PS10/PS11**; `Idempotency-Key` + idempotent `workflow_actions`; X.3 outbox |
| FR-PS09-014 Evidence vault | artefact ↔ document links, signatures | **PS13** document vault, **P05** read-audit |
| FR-PS09-015 Vigilance & sealed cover | integrity register, clearance, sealed cover | **P02** (confidential roles), feeds **PS06** promotion freeze |
| FR-PS09-016 SLA / timeline / analytics | stage SLA, pause-aware analytics | **P01** SLA runtime + **X.1** `JOB-PS09-SLA/TIMELINE`, **PS14** KPI feed |
| FR-PS09-017 Procedure-template overlay | two-layer overlay config | **W.1/W.2** flow/form definitions; SysAdmin = Org Admin |
| FR-PS09-018 Authority-competence guard | competence matrix + finalise guard | **P01** stage guard, **P02** |
| FR-PS09-019 Statutory consultation | UPSC/CVC/ICC/legal gating | **P01** (CONDITIONAL stage), **X.1** pause job, **PS13** advice |
| FR-PS09-020 Dispense-with-inquiry | Art. 311(2) provisos | **P01** (approval), **P05** speaking reasons |
| FR-PS09-021 Legal service & digital signing | valid service vs notification; DSC/eSign | **X.3** trust service, **PS13**, `legal_service_records`; X.2 informational only |
| FR-PS09-022 Multi-respondent proceedings | joint proceedings, per-respondent findings | **P01** (PARALLEL patterns), **P05** |
| FR-PS09-023 POSH / ICC procedure | ICC composition, conciliation | **P01** ICC flow, **P02** heightened confidentiality |
| FR-PS09-024 SLA pause/resume | clock management ledger | **X.1** `JOB-PS09-TIMELINE` recompute, **P01** SLA runtime |
| FR-PS09-025 Personal-hearing record | grant/denial record | **P01**, **P05** |
| FR-PS09-026 Jurisdiction transfer / sealed cover / retiree | DA re-resolution, Rule 9 bar | consumes **PS05** transfer event, feeds **PS06/PS11**, **P01** |
| FR-PS09-027 Tamper-evident audit & verify | court-grade verifiability | **P05 dual log** (DB-trigger, immutable, 7-yr) + **OPEN-PLAT-03** hash-chain (consumed, not re-built) |
| FR-PS09-028 Abatement on death | terminal handling, suppress effects | **P01** (terminal), **PS12** ABATED SR event, **P05** |

### `GAP (enterprise-specific)` engine authored

PS09 authors **one** net-new statutory engine — the **disciplinary due-process machine** (case → charge → inquiry → penalty → appeal with natural-justice kernel, Art. 311 competence, mandatory consultation, lawful service, DSC/eSign) — plus module-unique `VAL-PS09-*` rules, `JOB-PS09-*` jobs, `MSG-PS09-*` notifications and `ERR-PS09-*` error copy, all registered against the Foundation indexes. It authors **no** platform plumbing: workflow, RBAC, audit, notifications, jobs, migration and configured-content engines are consumed by id. The SR ledger it writes to is owned by **PS12**; payroll/pension/seniority effects are owned by **PS10/PS11/PS06**.

### New / renamed `VAL-`, `JOB-`, `MSG-`, `ERR-` ids (registered in Foundation indexes)

| Family | Ids (illustrative) | Purpose |
|--------|--------------------|---------|
| `VAL-PS09-*` | `VAL-PS09-DUEPROCESS`, `VAL-PS09-COMPETENCE`, `VAL-PS09-SERVICE`, `VAL-PS09-RECOVERYCAP` | charge→reply→inquiry→penalty sequencing; competence; valid service; recovery caps |
| `JOB-PS09-*` | `JOB-PS09-SLA`, `JOB-PS09-TIMELINE`, `JOB-PS09-EFFDATE` | stage SLA escalation; statutory-timeline pause/resume recompute; effective-dated penalty effects |
| `MSG-PS09-*` | `MSG-PS09-CHARGE-ISSUED`, `MSG-PS09-SHOWCAUSE`, `MSG-PS09-ORDER-SERVED`, `MSG-PS09-SLA-BREACH` | statutory notices (EMAIL mandatory per X.2/§9.9) |
| `ERR-PS09-*` | the §10.3 module codes (e.g. `ERR-PS09-AUTHORITY-NOT-COMPETENT`, `ERR-PS09-CONSULTATION-PENDING`) | domain refinements returned alongside the platform 8-code table |

---

## Amendments (v2 → v3: platform re-grounding)

> v3 preserves **all** v2 content, entities (E1–E30), FRs (28), integrity rules (DI-1…DI-29), state tables (9), sample data and rigor. The changes below re-anchor the module onto the PrimeSoft platform; no functional requirement was removed.

| # | Area | v2 (invented `SHARED_FOUNDATION`) | v3 (platform-grounded) | Authority |
|---|------|-----------------------------------|------------------------|-----------|
| A-1 | Module code | `M09-DCP` | **`PS09`** (cross-module refs re-keyed `M01/05/06/10/11/12/13/14 → PS0x`) | `MODULE_RECONCILIATION.md` §B |
| A-2 | Foundation source | `SHARED_FOUNDATION.md` §5 | **`PLATFORM_FOUNDATION.md`** + `MODULE_RECONCILIATION.md` §C | Foundation §1.1 |
| A-3 | Workflow engine | invented `workflow_instances`/`workflow_actions` | **P01 WorkflowEngine** (`workflows`/`workflow_instances`/`workflow_actions`); due-process = configured P01/W.1 flows; idempotent actions; in-flight version pinning | Platform §P01 |
| A-4 | Audit | invented `audit_log` + module-owned hash chain | **P05 dual log** (`audit_log` + `security_audit_log`), DB-trigger, immutable, PII-masked, ≥ 7-yr; tamper-evidence = **OPEN-PLAT-03** hash-chain (consumed); `case_timeline_events` rides on P05 | Platform §P05; §C |
| A-5 | RBAC | invented role list | **RBAC v1.7**: DA / IO / PO / Appellate / ICC etc. as **added roles + capability flags**; SoD enforced by P01/P02; Auditor → Org-Admin read; SysAdmin → Org/Platform Admin | RBAC §2/§4.3; §6.6 |
| A-6 | Confidentiality/masking | module field-level rules | **P02** PII Protection Ceiling + field mask on serialization; sealed-cover/confidential-source declared, enforcement consumed | Platform §P02 |
| A-7 | SR ledger | `service_register_events` as "shared platform entity" | **net-new PS12 SR ledger on the P05 substrate**; PS09 appends, effects by reference to PS06/PS10/PS11 | §C; §D |
| A-8 | API conventions | `/api/v1`, page/limit paging | `/api/v1`, **`Idempotency-Key`**, **cursor pagination** (limit 25/max 100, `next_cursor`), `?sort=`, **`X-Correlation-Id`** | Foundation §1 |
| A-9 | Error envelope | `{error{code,message,field}, requestId}` | `{error{code,message,field,details}}` + **`X-Correlation-Id` header** | Foundation §1; §C |
| A-10 | Error codes | `VALIDATION_ERROR(400)`, `AUTH_REQUIRED(401)`, `INTERNAL_ERROR(500)`, `UPSTREAM_UNAVAILABLE(503)` | **8-code table**: `VALIDATION_FAILED(422)`, `UNAUTHENTICATED(401)`, `FORBIDDEN`, `NOT_FOUND`, `CONFLICT`, `PRECONDITION_FAILED(412)`, `RATE_LIMITED`, `INTERNAL(500)` | Foundation §1; §C |
| A-11 | Multi-tenancy | omitted | **`tenant_id`/`entity_id`** on every entity; data-layer scoping; unscoped query rejected | Platform §0.1; §C |
| A-12 | Notifications | shared `notifications` (in-app/email/SMS) | **X.2** infra; EMAIL for statutory notices **mandatory/non-suppressible**; `MSG-PS09-*` templates | Platform §X.2; BRD §9.9 |
| A-13 | Background jobs / SLA | implicit SLA engine | **X.1** runner + `JOB-PS09-SLA/TIMELINE/EFFDATE`; P01 SLA runtime for stages | Platform §X.1; Foundation §4 |
| A-14 | Validation | inline rules | cite `VAL-*` by id; author `VAL-PS09-*` only | Foundation §2 |
| A-15 | NFR | 99.9% uptime, RPO ≤ 15 min | **99.5%/month, RPO < 1 h, RTO < 4 h**, p95 < 500 ms, WCAG 2.1 AA, soft-delete only | Vision §2.9; §C |
| A-16 | Migration | undefined | **P06** ETL+V, 3 dry runs, waves, `<enterprise>_source_id` | Platform §P06 |
| A-17 | New sections | — | **`## Alignment with PrimeSoft Platform`** (FR→service map) + this amendments table | Foundation §9 (6) |

---

## Amendments (v3 → v3.1: cross-module remediation)

> Cross-module remediation per `docs/review/REMEDIATION.md` (D3/D4) and findings R4/R5. v3.1 preserves all v3 content, entities, FRs, integrity rules and rigor; it only aligns identifier hygiene, removes invented-convention leaks from live sections, and re-attributes the statutory tamper-evidence mechanism owner. **PS09 SR event types/envelope are unchanged** (already conformant per D2).

| # | Decision | v3 (before) | v3.1 (after) | Authority |
| --- | --- | --- | --- | --- |
| B-1 | Error-code namespace (D3) | ~33 bare `UPPER_SNAKE` module codes in live §10.3 catalog + FR failure-handling tables (e.g. `AUTHORITY_NOT_COMPETENT`, `SIGNATURE_INVALID`, `IDEMPOTENCY_CONFLICT`) | **`ERR-PS09-*` namespace** for every module code, each mapped onto one of the 8 standard HTTP codes with a `MSG-PS09-*` message id (§10.3 mapping table) | REMEDIATION §D3; R4 Finding 1 |
| B-2 | `SIGNATURE_INVALID` collision (D3) | bare `SIGNATURE_INVALID` shared with PS12/PS13 | namespaced **`ERR-PS09-SIGNATURE-INVALID`** (collision resolved) | R4 Finding 3 |
| B-3 | `IDEMPOTENCY_CONFLICT` redefinition (D3) | bare 409 `IDEMPOTENCY_CONFLICT` duplicating platform `CONFLICT`/`ERR-DUP-INSTANCE` | **`ERR-PS09-IDEMPOTENCY-CONFLICT`** documented as a specialisation of `CONFLICT 409` / `ERR-DUP-INSTANCE` | R4 Finding 6 |
| B-4 | Invented `VALIDATION_ERROR` leak (D4) | live §2251 "kernel-disable ⇒ 422 `VALIDATION_ERROR`" | mapped to standard **`VALIDATION_FAILED` (422)** | REMEDIATION §D4; R5 Finding 1 |
| B-5 | Invented `503`/`UPSTREAM_UNAVAILABLE` leaks (D4) | live FR tables returned `503` (PDF render, PS13 down) and `UPSTREAM_UNAVAILABLE` (PS10/PS01 down) | mapped to **`INTERNAL 500` / `ERR-LOADFAIL`** with X.3 outbox retry; `503`/`UPSTREAM_UNAVAILABLE` retained only in the v2→v3 override row A-10 | REMEDIATION §D4; R5 Finding 1 |
| B-6 | Parallel module hash-chain mirror (D4) | ownership table implied a PS09-owned "module mirror hash-chained in E19" as the audit record | statutory immutability/tamper-evidence is the **P05 dual-log (DB-trigger) + OPEN-PLAT-03 hash-chaining**, not a PS09-owned chain; E19 `case_timeline_events` is a per-case SLA/stage ledger riding on the P05 substrate (requirement kept, mechanism owner changed) | REMEDIATION §D4; R5 Finding 5 |
| B-7 | SR write data-op wording (F-12) | FR data-ops wrote `INSERT service_register_events` directly | reworded to **append via the PS12 ingestion contract `POST /api/v1/sr/ingest` (FR-02)** in FR-003/011/012/013/028; event types/envelope unchanged | REMEDIATION §D1/D4 (PS09 row) |
...
| Penalty | PS06 | PS10 | PS11 | PS12 | Competence (Art. 311(1)) |
|---------|-----|-----|-----|-----|--------------------------|
...
|------|----------|-------------------|
| R1 Authority competence | Critical | FR-018 + DI-13 + `authority_competence` + `ERR-PS09-AUTHORITY-NOT-COMPETENT` |
| R2 Mandatory consultation | Critical | FR-019 + DI-14 + `case_consultations` + `ERR-PS09-CONSULTATION-PENDING` |
| R3 Dispense-with-inquiry | High | FR-020 + DI-23 + `inquiry_dispensations` |
| R4 Document/witness inspection | High | FR-007 + DI-24 + `ERR-PS09-INSPECTION-NOT-AFFORDED` |
...
- The penalty taxonomy, authority hierarchy, and competence matrix are reference data resolved from org configuration (PS01/org_units), `authority_competence`, and the conduct-rules master.
- DSC/eSign is provided by a platform trust service; PS09 stores returned signature metadata and verifies on demand.
