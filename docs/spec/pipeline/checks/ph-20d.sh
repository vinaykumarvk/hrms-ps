#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-7 verdict — cites the tranche-6 verdict + coverage delta,
...
V=docs/spec/ph-20-verdict.md
echo "== PH-18D exit-criteria (tranche-7 verdict; human gate) =="
...
must "verdict chains from tranche-6 verdict::ph-19-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS02 PS07 PS13; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-20D met (await HUMAN gate review)' || echo 'RED - PH-20D not complete') =="; exit "$fail"
