#!/usr/bin/env bash
# PH-54B oracle (human gate): honest tranche-41 verdict — cites the tranche-40 verdict + coverage delta,
# names the coverage ratchet (504->513 / 38.8%) and the PS05 transfer/counselling read route exposure,
...
V=docs/spec/ph-54-verdict.md
echo "== PH-54B exit-criteria (tranche-41 verdict; human gate) =="
...
must "verdict chains from tranche-40 verdict::ph-53-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
must "verdict names the coverage ratchet::513|38.8%|transfer|counselling" "$V"
must "verdict carries a PS05 row::PS05" "$V"
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-54B met (await HUMAN gate review)' || echo 'RED - PH-54B not complete') =="; exit "$fail"
