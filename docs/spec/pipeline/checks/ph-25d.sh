#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-12 verdict — cites the tranche-11 verdict + coverage delta,
...
V=docs/spec/ph-25-verdict.md
echo "== PH-18D exit-criteria (tranche-12 verdict; human gate) =="
...
must "verdict chains from tranche-11 verdict::ph-24-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS02 PS03 PS10; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-25D met (await HUMAN gate review)' || echo 'RED - PH-25D not complete') =="; exit "$fail"
