#!/usr/bin/env bash
# PH-18D oracle (human gate): honest tranche-17 verdict — cites the tranche-16 verdict + coverage delta,
...
V=docs/spec/ph-30-verdict.md
echo "== PH-18D exit-criteria (tranche-17 verdict; human gate) =="
...
must "verdict chains from tranche-16 verdict::ph-29-verdict" "$V"
must "verdict cites the coverage delta::brd-coverage-delta-20260703" "$V"
for m in PS01 PS03 PS08; do must "verdict carries a $m row::$m" "$V"; done
...
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-30D met (await HUMAN gate review)' || echo 'RED - PH-30D not complete') =="; exit "$fail"
